...
\begin{funcdefna}{MPI\_INIT}{()}
\end{funcdefna}
\mpibindmain{MPI\_Init}{(\gb{}int~*argc, char~***argv)}
\mpifnewbindmain{MPI\_Init}{(ierror) \fargs \fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_INIT}{(IERROR) \fargs \fnfinalarg{}INTEGER \mbox{IERROR}}
...
\begin{funcdef}{MPI\_INIT\_THREAD}{(\mbox{required}, \mbox{provided})}
\funcarg{\IN}{required}{desired level of thread support (integer)}
\funcarg{\OUT}{provided}{provided level of thread support (integer)}
\end{funcdef}
\mpibindmain{MPI\_Init\_thread}{(\gb{}int~*argc, char~***argv, int~required, int~*provided)}
\mpifnewbindmain{MPI\_Init\_thread}{(required, provided, ierror) \fargs INTEGER, INTENT(IN)\ ::\ \mbox{required}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{provided}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_INIT\_THREAD}{(REQUIRED, PROVIDED, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{REQUIRED}, \mbox{PROVIDED}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_QUERY\_THREAD}{(\mbox{provided})}
\funcarg{\OUT}{provided}{provided level of thread support (integer)}
\end{funcdef}
\mpibindmain{MPI\_Query\_thread}{(\gb{}int~*provided)}
\mpifnewbindmain{MPI\_Query\_thread}{(provided, ierror) \fargs INTEGER, INTENT(OUT)\ ::\ \mbox{provided}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_QUERY\_THREAD}{(PROVIDED, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{PROVIDED}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_IS\_THREAD\_MAIN}{(\mbox{flag})}
\funcarg{\OUT}{flag}{true if calling thread is main thread, false otherwise (logical)}
\end{funcdef}
\mpibindmain{MPI\_Is\_thread\_main}{(\gb{}int~*flag)}
\mpifnewbindmain{MPI\_Is\_thread\_main}{(flag, ierror) \fargs LOGICAL, INTENT(OUT)\ ::\ \mbox{flag}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_IS\_THREAD\_MAIN}{(FLAG, IERROR) \fargs LOGICAL \mbox{FLAG}\fnfinalarg{}INTEGER \mbox{IERROR}}
...
\begin{funcdefna}{MPI\_FINALIZE}{()}
\end{funcdefna}
\mpibindmain{MPI\_Finalize}{(void)}
\mpifnewbindmain{MPI\_Finalize}{(ierror) \fargs \fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_FINALIZE}{(IERROR) \fargs \fnfinalarg{}INTEGER \mbox{IERROR}}
...
\begin{funcdef}{MPI\_INITIALIZED}{(\mbox{flag})}
\funcarg{\OUT}{flag}{Flag is true if \mpifunc{MPI\_INIT} or \mpifunc{MPI\_INIT\_THREAD} has been called and false otherwise (logical)}
\end{funcdef}
\mpibindmain{MPI\_Initialized}{(\gb{}int~*flag)}
\mpifnewbindmain{MPI\_Initialized}{(flag, ierror) \fargs LOGICAL, INTENT(OUT)\ ::\ \mbox{flag}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_INITIALIZED}{(FLAG, IERROR) \fargs LOGICAL \mbox{FLAG}\fnfinalarg{}INTEGER \mbox{IERROR}}
...
\begin{funcdef}{MPI\_FINALIZED}{(\mbox{flag})}
\funcarg{\OUT}{flag}{true if \mpifunc{MPI\_FINALIZE} has been called and false otherwise. (logical)}
\end{funcdef}
\mpibindmain{MPI\_Finalized}{(\gb{}int~*flag)}
\mpifnewbindmain{MPI\_Finalized}{(flag, ierror) \fargs LOGICAL, INTENT(OUT)\ ::\ \mbox{flag}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_FINALIZED}{(FLAG, IERROR) \fargs LOGICAL \mbox{FLAG}\fnfinalarg{}INTEGER \mbox{IERROR}}
...
\begin{figure}[ht]
  \centerline{\includegraphics[width=3.50in]{figures/session_flow}}
...
\begin{funcdef}{MPI\_SESSION\_INIT}{(\mbox{info}, \mbox{errhandler}, \mbox{session})}
\funcarg{\IN}{info}{info object to specify thread support level and \MPI/ implementation specific resources (handle)}
\funcarg{\IN}{errhandler}{error handler to invoke in the event that an error is encountered during this function call (handle)}
\funcarg{\OUT}{session}{new session (handle)}
\end{funcdef}
\mpibindmain{MPI\_Session\_init}{(\gb{}MPI\_Info~info, MPI\_Errhandler~errhandler, MPI\_Session~*session)}
\mpifnewbindmain{MPI\_Session\_init}{(info, errhandler, session, ierror) \fargs TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Errhandler), INTENT(IN)\ ::\ \mbox{errhandler}\fnarg{}TYPE(MPI\_Session), INTENT(OUT)\ ::\ \mbox{session}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_SESSION\_INIT}{(INFO, ERRHANDLER, SESSION, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{INFO}, \mbox{ERRHANDLER}, \mbox{SESSION}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_SESSION\_FINALIZE}{(\mbox{session})}
\funcarg{\INOUT}{session}{session to be finalized (handle)}
\end{funcdef}
\mpibindmain{MPI\_Session\_finalize}{(\gb{}MPI\_Session~*session)}
\mpifnewbindmain{MPI\_Session\_finalize}{(session, ierror) \fargs TYPE(MPI\_Session), INTENT(INOUT)\ ::\ \mbox{session}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_SESSION\_FINALIZE}{(SESSION, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{SESSION}, \mbox{IERROR}}
...
\begin{figure}
  \centerline{\includegraphics[width=4.50in]{figures/figure_pset}}
...
\begin{funcdef}{MPI\_SESSION\_GET\_NUM\_PSETS}{(\mbox{session}, \mbox{info}, \mbox{npset\_names})}
\funcarg{\IN}{session}{session (handle)}
\funcarg{\IN}{info}{info object (handle)}
\funcarg{\OUT}{npset\_names}{number of available process sets (nonnegative integer)}
\end{funcdef}
\mpibindmain{MPI\_Session\_get\_num\_psets}{(\gb{}MPI\_Session~session, MPI\_Info~info, int~*npset\_names)}
\mpifnewbindmain{MPI\_Session\_get\_num\_psets}{(session, info, npset\_names, ierror) \fargs TYPE(MPI\_Session), INTENT(IN)\ ::\ \mbox{session}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{npset\_names}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_SESSION\_GET\_NUM\_PSETS}{(SESSION, INFO, NPSET\_NAMES, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{SESSION}, \mbox{INFO}, \mbox{NPSET\_NAMES}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_SESSION\_GET\_NTH\_PSET}{(\mbox{session}, \mbox{info}, \mbox{n}, \mbox{pset\_len}, \mbox{pset\_name})}
\funcarg{\IN}{session}{session (handle)}
\funcarg{\IN}{info}{info object (handle)}
\funcarg{\IN}{n}{index of the desired process set name (integer)}
\funcarg{\INOUT}{pset\_len}{length of the pset\_name argument (integer)}
\funcarg{\OUT}{pset\_name}{name of the \mpiarg{n}th process set (string)}
\end{funcdef}
\mpibindmain{MPI\_Session\_get\_nth\_pset}{(\gb{}MPI\_Session~session, MPI\_Info~info, int~n, int~*pset\_len, char~*pset\_name)}
\mpifnewbindmain{MPI\_Session\_get\_nth\_pset}{(session, info, n, pset\_len, pset\_name, ierror) \fargs TYPE(MPI\_Session), INTENT(IN)\ ::\ \mbox{session}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{n}\fnarg{}INTEGER, INTENT(INOUT)\ ::\ \mbox{pset\_len}\fnarg{}CHARACTER(LEN=*), INTENT(OUT)\ ::\ \mbox{pset\_name}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_SESSION\_GET\_NTH\_PSET}{(SESSION, INFO, N, PSET\_LEN, PSET\_NAME, IERROR) \fargs INTEGER \mbox{SESSION}, \mbox{INFO}, \mbox{N}, \mbox{PSET\_LEN}, \mbox{IERROR}\fnfinalarg{}CHARACTER*(*) \mbox{PSET\_NAME}}
...
\begin{funcdef}{MPI\_SESSION\_GET\_INFO}{(\mbox{session}, \mbox{info\_used})}
\funcarg{\IN}{session}{session (handle)}
\funcarg{\OUT}{info\_used}{see explanation below (handle)}
\end{funcdef}
\mpibindmain{MPI\_Session\_get\_info}{(\gb{}MPI\_Session~session, MPI\_Info~*info\_used)}
\mpifnewbindmain{MPI\_Session\_get\_info}{(session, info\_used, ierror) \fargs TYPE(MPI\_Session), INTENT(IN)\ ::\ \mbox{session}\fnarg{}TYPE(MPI\_Info), INTENT(OUT)\ ::\ \mbox{info\_used}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_SESSION\_GET\_INFO}{(SESSION, INFO\_USED, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{SESSION}, \mbox{INFO\_USED}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_SESSION\_GET\_PSET\_INFO}{(\mbox{session}, \mbox{pset\_name}, \mbox{info})}
\funcarg{\IN}{session}{session (handle)}
\funcarg{\IN}{pset\_name}{name of process set (string)}
\funcarg{\OUT}{info}{info object containing information about the given process set (handle)}
\end{funcdef}
\mpibindmain{MPI\_Session\_get\_pset\_info}{(\gb{}MPI\_Session~session, const~char~*pset\_name, MPI\_Info~*info)}
\mpifnewbindmain{MPI\_Session\_get\_pset\_info}{(session, pset\_name, info, ierror) \fargs TYPE(MPI\_Session), INTENT(IN)\ ::\ \mbox{session}\fnarg{}CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{pset\_name}\fnarg{}TYPE(MPI\_Info), INTENT(OUT)\ ::\ \mbox{info}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_SESSION\_GET\_PSET\_INFO}{(SESSION, PSET\_NAME, INFO, IERROR) \fargs INTEGER \mbox{SESSION}, \mbox{INFO}, \mbox{IERROR}\fnfinalarg{}CHARACTER*(*) \mbox{PSET\_NAME}}
...
\begin{funcdef}{MPI\_ABORT}{(\mbox{comm}, \mbox{errorcode})}
\funcarg{\IN}{comm}{communicator of \MPI/ processes to abort (handle)}
\funcarg{\IN}{errorcode}{error code to return to invoking environment (integer)}
\end{funcdef}
\mpibindmain{MPI\_Abort}{(\gb{}MPI\_Comm~comm, int~errorcode)}
\mpifnewbindmain{MPI\_Abort}{(comm, errorcode, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{errorcode}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_ABORT}{(COMM, ERRORCODE, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{ERRORCODE}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_COMM\_SPAWN}{(\mbox{command}, \mbox{argv}, \mbox{maxprocs}, \mbox{info}, \mbox{root}, \mbox{comm}, \mbox{intercomm}, \mbox{array\_of\_errcodes})}
\funcarg{\IN}{command}{name of program to be spawned (string, significant only at root)}
\funcarg{\IN}{argv}{arguments to \mpiarg{command} (array of strings, significant only at root)}
\funcarg{\IN}{maxprocs}{maximum number of processes to start (integer, significant only at root)}
\funcarg{\IN}{info}{a set of key-value pairs telling the runtime system where and how to start the processes (handle, significant only at root)}
\funcarg{\IN}{root}{rank of process in which previous arguments are examined (integer)}
\funcarg{\IN}{comm}{intra-communicator containing group of spawning processes (handle)}
\funcarg{\OUT}{intercomm}{inter-communicator between original group and the newly spawned group (handle)}
\funcarg{\OUT}{array\_of\_errcodes}{one code per process (array of integers)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_spawn}{(\gb{}const~char~*command, char~*argv[], int~maxprocs, MPI\_Info~info, int~root, MPI\_Comm~comm, MPI\_Comm~*intercomm, int~array\_of\_errcodes[])}
\mpifnewbindmain{MPI\_Comm\_spawn}{(command, argv, maxprocs, info, root, comm, intercomm, array\_of\_errcodes, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{command}, \mbox{argv(*)}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{maxprocs}, \mbox{root}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{intercomm}\fnarg{}INTEGER\ ::\ \mbox{array\_of\_errcodes(*)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_SPAWN}{(COMMAND, ARGV, MAXPROCS, INFO, ROOT, COMM, INTERCOMM, ARRAY\_OF\_ERRCODES, IERROR) \fargs CHARACTER*(*) \mbox{COMMAND}, \mbox{ARGV(*)}\fnfinalarg{}INTEGER \mbox{MAXPROCS}, \mbox{INFO}, \mbox{ROOT}, \mbox{COMM}, \mbox{INTERCOMM}, \mbox{ARRAY\_OF\_ERRCODES(*)}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_COMM\_GET\_PARENT}{(\mbox{parent})}
\funcarg{\OUT}{parent}{the parent communicator (handle)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_get\_parent}{(\gb{}MPI\_Comm~*parent)}
\mpifnewbindmain{MPI\_Comm\_get\_parent}{(parent, ierror) \fargs TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{parent}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_GET\_PARENT}{(PARENT, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{PARENT}, \mbox{IERROR}}
...
\cdeclindex{MPI\_Info}%
\begin{funcdef}{MPI\_COMM\_SPAWN\_MULTIPLE}{(\mbox{count}, \mbox{array\_of\_commands}, \mbox{array\_of\_argv}, \mbox{array\_of\_maxprocs}, \mbox{array\_of\_info}, \mbox{root}, \mbox{comm}, \mbox{intercomm}, \mbox{array\_of\_errcodes})}
\funcarg{\IN}{count}{number of commands (positive integer, significant only at root)}
\funcarg{\IN}{array\_of\_commands}{programs to be executed (array of strings, significant only at root)}
\funcarg{\IN}{array\_of\_argv}{arguments for \mpiarg{commands} (array of array of strings, significant only at root)}
\funcarg{\IN}{array\_of\_maxprocs}{maximum number of processes to start for each command (array of integers, significant only at root)}
\funcarg{\IN}{array\_of\_info}{info objects telling the runtime system where and how to start processes (array of handles, significant only at root)}
\funcarg{\IN}{root}{rank of process in which previous arguments are examined (integer)}
\funcarg{\IN}{comm}{intra-communicator containing group of spawning processes (handle)}
\funcarg{\OUT}{intercomm}{inter-communicator between original group and the newly spawned group (handle)}
\funcarg{\OUT}{array\_of\_errcodes}{one error code per process (array of integers)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_spawn\_multiple}{(\gb{}int~count, char~*array\_of\_commands[], char~**array\_of\_argv[], const~int~array\_of\_maxprocs[], const~MPI\_Info~array\_of\_info[], int~root, MPI\_Comm~comm, MPI\_Comm~*intercomm, int~array\_of\_errcodes[])}
\mpifnewbindmain{MPI\_Comm\_spawn\_multiple}{(count, array\_of\_commands, array\_of\_argv, array\_of\_maxprocs, array\_of\_info, root, comm, intercomm, array\_of\_errcodes, ierror) \fargs INTEGER, INTENT(IN)\ ::\ \mbox{count}, \mbox{array\_of\_maxprocs(*)}, \mbox{root}\fnarg{}CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{array\_of\_commands(*)}, \mbox{array\_of\_argv(count, *)}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{array\_of\_info(*)}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{intercomm}\fnarg{}INTEGER\ ::\ \mbox{array\_of\_errcodes(*)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_SPAWN\_MULTIPLE}{(COUNT, ARRAY\_OF\_COMMANDS, ARRAY\_OF\_ARGV, ARRAY\_OF\_MAXPROCS, ARRAY\_OF\_INFO, ROOT, COMM, INTERCOMM, ARRAY\_OF\_ERRCODES, IERROR) \fargs INTEGER \mbox{COUNT}, \mbox{ARRAY\_OF\_MAXPROCS(*)}, \mbox{ARRAY\_OF\_INFO(*)}, \mbox{ROOT}, \mbox{COMM}, \mbox{INTERCOMM}, \mbox{ARRAY\_OF\_ERRCODES(*)}, \mbox{IERROR}\fnfinalarg{}CHARACTER*(*) \mbox{ARRAY\_OF\_COMMANDS(*)}, \mbox{ARRAY\_OF\_ARGV(COUNT, *)}}
...
\cdeclindex{MPI\_Info}%
\begin{funcdef}{MPI\_OPEN\_PORT}{(\mbox{info}, \mbox{port\_name})}
\funcarg{\IN}{info}{implementation-specific information on how to establish an address (handle)}
\funcarg{\OUT}{port\_name}{newly established port (string)}
\end{funcdef}
\mpibindmain{MPI\_Open\_port}{(\gb{}MPI\_Info~info, char~*port\_name)}
\mpifnewbindmain{MPI\_Open\_port}{(info, port\_name, ierror) \fargs TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}CHARACTER(LEN=MPI\_MAX\_PORT\_NAME), INTENT(OUT)\ ::\ \mbox{port\_name}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_OPEN\_PORT}{(INFO, PORT\_NAME, IERROR) \fargs INTEGER \mbox{INFO}, \mbox{IERROR}\fnfinalarg{}CHARACTER*(*) \mbox{PORT\_NAME}}
...
\begin{funcdef}{MPI\_CLOSE\_PORT}{(\mbox{port\_name})}
\funcarg{\IN}{port\_name}{a port (string)}
\end{funcdef}
\mpibindmain{MPI\_Close\_port}{(\gb{}const~char~*port\_name)}
\mpifnewbindmain{MPI\_Close\_port}{(port\_name, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{port\_name}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CLOSE\_PORT}{(PORT\_NAME, IERROR) \fargs CHARACTER*(*) \mbox{PORT\_NAME}\fnfinalarg{}INTEGER \mbox{IERROR}}

This function releases the network address represented by
\mpiarg{port\_name}.

\begin{funcdef}{MPI\_COMM\_ACCEPT}{(\mbox{port\_name}, \mbox{info}, \mbox{root}, \mbox{comm}, \mbox{newcomm})}
\funcarg{\IN}{port\_name}{port name (string, significant only at root)}
\funcarg{\IN}{info}{implementation-dependent information (handle, significant only at root)}
\funcarg{\IN}{root}{rank of root in \mpiarg{comm} (integer)}
\funcarg{\IN}{comm}{intra-communicator over which call is collective (handle)}
\funcarg{\OUT}{newcomm}{inter-communicator with client as remote group (handle)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_accept}{(\gb{}const~char~*port\_name, MPI\_Info~info, int~root, MPI\_Comm~comm, MPI\_Comm~*newcomm)}
\mpifnewbindmain{MPI\_Comm\_accept}{(port\_name, info, root, comm, newcomm, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{port\_name}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{root}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{newcomm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_ACCEPT}{(PORT\_NAME, INFO, ROOT, COMM, NEWCOMM, IERROR) \fargs CHARACTER*(*) \mbox{PORT\_NAME}\fnfinalarg{}INTEGER \mbox{INFO}, \mbox{ROOT}, \mbox{COMM}, \mbox{NEWCOMM}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_COMM\_CONNECT}{(\mbox{port\_name}, \mbox{info}, \mbox{root}, \mbox{comm}, \mbox{newcomm})}
\funcarg{\IN}{port\_name}{network address (string, significant only at root)}
\funcarg{\IN}{info}{implementation-dependent information (handle, significant only at root)}
\funcarg{\IN}{root}{rank of root in \mpiarg{comm} (integer)}
\funcarg{\IN}{comm}{intra-communicator over which call is collective (handle)}
\funcarg{\OUT}{newcomm}{inter-communicator with server as remote group (handle)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_connect}{(\gb{}const~char~*port\_name, MPI\_Info~info, int~root, MPI\_Comm~comm, MPI\_Comm~*newcomm)}
\mpifnewbindmain{MPI\_Comm\_connect}{(port\_name, info, root, comm, newcomm, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{port\_name}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{root}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{newcomm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_CONNECT}{(PORT\_NAME, INFO, ROOT, COMM, NEWCOMM, IERROR) \fargs CHARACTER*(*) \mbox{PORT\_NAME}\fnfinalarg{}INTEGER \mbox{INFO}, \mbox{ROOT}, \mbox{COMM}, \mbox{NEWCOMM}, \mbox{IERROR}}
...
\cdeclindex{MPI\_Info}%
\begin{funcdef}{MPI\_PUBLISH\_NAME}{(\mbox{service\_name}, \mbox{info}, \mbox{port\_name})}
\funcarg{\IN}{service\_name}{a service name to associate with the port (string)}
\funcarg{\IN}{info}{implementation-specific information (handle)}
\funcarg{\IN}{port\_name}{a port name (string)}
\end{funcdef}
\mpibindmain{MPI\_Publish\_name}{(\gb{}const~char~*service\_name, MPI\_Info~info, const~char~*port\_name)}
\mpifnewbindmain{MPI\_Publish\_name}{(service\_name, info, port\_name, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{service\_name}, \mbox{port\_name}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_PUBLISH\_NAME}{(SERVICE\_NAME, INFO, PORT\_NAME, IERROR) \fargs CHARACTER*(*) \mbox{SERVICE\_NAME}, \mbox{PORT\_NAME}\fnfinalarg{}INTEGER \mbox{INFO}, \mbox{IERROR}}
...
\cdeclindex{MPI\_Info}%
\begin{funcdef}{MPI\_UNPUBLISH\_NAME}{(\mbox{service\_name}, \mbox{info}, \mbox{port\_name})}
\funcarg{\IN}{service\_name}{a service name (string)}
\funcarg{\IN}{info}{implementation-specific information (handle)}
\funcarg{\IN}{port\_name}{a port name (string)}
\end{funcdef}
\mpibindmain{MPI\_Unpublish\_name}{(\gb{}const~char~*service\_name, MPI\_Info~info, const~char~*port\_name)}
\mpifnewbindmain{MPI\_Unpublish\_name}{(service\_name, info, port\_name, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{service\_name}, \mbox{port\_name}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_UNPUBLISH\_NAME}{(SERVICE\_NAME, INFO, PORT\_NAME, IERROR) \fargs CHARACTER*(*) \mbox{SERVICE\_NAME}, \mbox{PORT\_NAME}\fnfinalarg{}INTEGER \mbox{INFO}, \mbox{IERROR}}
...
\cdeclindex{MPI\_Info}%
\begin{funcdef}{MPI\_LOOKUP\_NAME}{(\mbox{service\_name}, \mbox{info}, \mbox{port\_name})}
\funcarg{\IN}{service\_name}{a service name (string)}
\funcarg{\IN}{info}{implementation-specific information (handle)}
\funcarg{\OUT}{port\_name}{a port name (string)}
\end{funcdef}
\mpibindmain{MPI\_Lookup\_name}{(\gb{}const~char~*service\_name, MPI\_Info~info, char~*port\_name)}
\mpifnewbindmain{MPI\_Lookup\_name}{(service\_name, info, port\_name, ierror) \fargs CHARACTER(LEN=*), INTENT(IN)\ ::\ \mbox{service\_name}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}CHARACTER(LEN=MPI\_MAX\_PORT\_NAME), INTENT(OUT)\ ::\ \mbox{port\_name}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_LOOKUP\_NAME}{(SERVICE\_NAME, INFO, PORT\_NAME, IERROR) \fargs CHARACTER*(*) \mbox{SERVICE\_NAME}, \mbox{PORT\_NAME}\fnfinalarg{}INTEGER \mbox{INFO}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_COMM\_DISCONNECT}{(\mbox{comm})}
\funcarg{\INOUT}{comm}{communicator (handle)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_disconnect}{(\gb{}MPI\_Comm~*comm)}
\mpifnewbindmain{MPI\_Comm\_disconnect}{(comm, ierror) \fargs TYPE(MPI\_Comm), INTENT(INOUT)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_DISCONNECT}{(COMM, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{IERROR}}
...
\cdeclindex{MPI\_Info}%
\begin{funcdef}{MPI\_COMM\_JOIN}{(\mbox{fd}, \mbox{intercomm})}
\funcarg{\IN}{fd}{socket file descriptor}
\funcarg{\OUT}{intercomm}{new inter-communicator (handle)}
\end{funcdef}
\mpibindmain{MPI\_Comm\_join}{(\gb{}int~fd, MPI\_Comm~*intercomm)}
\mpifnewbindmain{MPI\_Comm\_join}{(fd, intercomm, ierror) \fargs INTEGER, INTENT(IN)\ ::\ \mbox{fd}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{intercomm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_COMM\_JOIN}{(FD, INTERCOMM, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{FD}, \mbox{INTERCOMM}, \mbox{IERROR}}
