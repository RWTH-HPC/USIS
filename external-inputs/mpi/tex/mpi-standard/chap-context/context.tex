...
\begin{mpi-binding}
    function_name("MPI_Group_size")
    
    parameter("group", "GROUP", desc=r"group")
    parameter(
        "size",
        "COMM_SIZE",
        direction="out",
        desc=r"number of \MPI/ processes in the group",
    )
\end{mpi-binding}

\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_rank")
    ...
    parameter(
        ...
        direction="out",
        desc=(
            r"rank of the calling \MPI/ process in group, or "
            r"\mpiconst{MPI_UNDEFINED} if the \MPI/ process is not a member"
        ),
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_translate_ranks")
    
    parameter("group1", "GROUP", desc=r"group1")
    
    # this is the length of the ranks1 and ranks2 arrays
    parameter(
        "n",
        "ARRAY_LENGTH",
        desc=r"number of elements in \mpiarg{ranks1} and \mpiarg{ranks2} arrays",
    )
    parameter(
        "ranks1",
        "RANK",
        length="n",
        constant=True,
        desc=r"array of zero or more valid ranks in group1",
        suppress="lis_paren",
    )
    parameter("group2", "GROUP", desc=r"group2")
    parameter(
        "ranks2",
        "RANK",
        length="n",
        direction="out",
        desc=(
            r"array of corresponding ranks in group2, \mpiconst{MPI_UNDEFINED} "
            r"when no correspondence exists."
        ),
        suppress="lis_paren",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_compare")
    
    parameter("group1", "GROUP", desc=r"first group")
    parameter("group2", "GROUP", desc=r"second group")
    parameter("result", "GROUP_COMPARISON", direction="out", desc=r"result")
\end{mpi-binding}
...
\begin{mpi-binding}
    ...
    parameter(
        ...
        "GROUP",
        direction="out",
        desc=r"group corresponding to \mpiarg{comm}",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_union")
    
    parameter("group1", "GROUP", desc=r"first group")
    parameter("group2", "GROUP", desc=r"second group")
    parameter("newgroup", "GROUP", direction="out", desc=r"union group")
\end{mpi-binding}

\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_intersection")
    
    parameter("group1", "GROUP", desc=r"first group")
    parameter("group2", "GROUP", desc=r"second group")
    parameter("newgroup", "GROUP", direction="out", desc=r"intersection group")
\end{mpi-binding}

\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_difference")
    
    parameter("group1", "GROUP", desc=r"first group")
    parameter("group2", "GROUP", desc=r"second group")
    parameter("newgroup", "GROUP", direction="out", desc=r"difference group")
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_incl")
    
    parameter("group", "GROUP", desc=r"group")
    parameter(
        "n",
        "ARRAY_LENGTH",
        desc=r"number of elements in array \mpiarg{ranks} (and size of \mpiarg{newgroup})",
    )
    parameter(
        "ranks",
        "RANK",
        length="n",
        constant=True,
        desc=r"ranks of processes in \mpiarg{group} to appear in \mpiarg{newgroup}",
    )
    parameter(
        "newgroup",
        "GROUP",
        direction="out",
        desc=r"new group derived from above, in the order defined by \mpiarg{ranks}",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_excl")
    
    parameter("group", "GROUP", desc=r"group")
    parameter("n", "ARRAY_LENGTH", desc=r"number of elements in array \mpiarg{ranks}")
    parameter(
        "ranks",
        "RANK",
        length="n",
        constant=True,
        desc=r"array of integer ranks of \MPI/ processes in \mpiarg{group} not to appear in \mpiarg{newgroup}",
        suppress="lis_paren",
    )
    parameter(
        "newgroup",
        "GROUP",
        direction="out",
        desc=r"new group derived from above, preserving the order defined by \mpiarg{group}",
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    ...
    parameter("group", "GROUP", desc=r"group")
    parameter(
        "n", "ARRAY_LENGTH", desc=r"number of triplets in array \mpiarg{ranges}"
    )
    parameter(
        "ranges",
        "RANK",
        length=["n", "3"],
        desc=(
            r"a one-dimensional array of integer triplets, of the form "
            r"(first rank, last rank, stride) indicating ranks in "
            r"\mpiarg{group} of \MPI/ processes to be included in "
            r"\mpiarg{newgroup}"
        ),
        suppress="lis_paren",
    )
    parameter(
        ...
        direction="out",
        desc=(
            r"new group derived from above, in the order defined by "
            r"\mpiarg{ranges}"
        ),
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_range_excl")
    ...
    parameter(
        ...
        desc=(
            r"a one-dimensional array of integer triplets, of the form "
            r"(first rank, last rank, stride) indicating ranks in "
            r"\mpiarg{group} of \MPI/ processes to be excluded from the output "
            r"group \mpiarg{newgroup}"
        ),
    )
    parameter(
        "newgroup",
        "GROUP",
        direction="out",
        desc=r"new group derived from above, preserving the order in \mpiarg{group}",
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Group_from_session_pset")
    parameter("session", "SESSION", desc=r"session")
    parameter("pset_name", "STRING", constant=True, desc=r"name of process set to use to create the new group")
    parameter("newgroup", "GROUP", direction="out", desc=r"new group derived from supplied session and process set")
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Group_free")
    
    parameter("group", "GROUP", direction="inout", desc=r"group")
\end{mpi-binding}
...
\begin{mpi-binding}
    ...
    parameter(
        ...
        "COMM_SIZE",
        direction="out",
        desc=r"number of \MPI/ processes in the group of \mpiarg{comm}",
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    ...
    parameter(
        ...
        "RANK",
        direction="out",
        desc=r"rank of the calling \MPI/ process in group of \mpiarg{comm}",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_compare")
    
    parameter("comm1", "COMMUNICATOR", desc=r"first communicator")
    parameter("comm2", "COMMUNICATOR", desc=r"second communicator")
    parameter("result", "COMM_COMPARISON", direction="out", desc=r"result")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_dup_with_info")
...
\begin{mpi-binding}
    ...
    parameter(
        "request", "REQUEST", direction="out", desc=r"communication request"
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_idup_with_info")
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_create")
    
    parameter("comm", "COMMUNICATOR", desc=r"communicator")
    parameter(
        "group",
        "GROUP",
        desc=r"group, which is a subset of the group of \mpiarg{comm}",
    )
    parameter(
        "newcomm", "COMMUNICATOR", direction="out", desc=r"new communicator"
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Comm_create_group")
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_split")
    
    parameter("comm", "COMMUNICATOR", desc=r"communicator")
    parameter("color", "COLOR", desc=r"control of subset assignment")
    parameter("key", "KEY", desc=r"control of rank assignment")
    parameter(
        "newcomm", "COMMUNICATOR", direction="out", desc=r"new communicator"
    )
\end{mpi-binding}
...
\begin{figure}[ht]
  \centerline{\includegraphics[width=3.5in]{figures/collective-split2}}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_split_type")
    
    parameter("comm", "COMMUNICATOR", desc=r"communicator")
    parameter(
        "split_type",
        "SPLIT_TYPE",
        desc=r"type of processes to be grouped together",
    )
    parameter("key", "KEY", desc=r"control of rank assignment")
    parameter("info", "INFO", desc=r"info argument", direction='lis:inout,param:in')
    parameter(
        "newcomm", "COMMUNICATOR", direction="out", desc=r"new communicator"
    )
\end{mpi-binding}
...
type instance---and no other instance of the same hardware resource type---are included in the group of
  ...
  \begin{figure}[htpb]
    \centerline{\includegraphics[width=6.0in]{figures/recursive_split}}
...
\begin{mpi-binding}
    function_name("MPI_Comm_create_from_group")
    parameter("group", "GROUP", desc=r"group")
    parameter("stringtag", "STRING", constant=True, desc=r"unique identifier for this operation")
    parameter("info", "INFO", desc=r"info object")
    parameter("errhandler", "ERRHANDLER", desc=r"error handler to be attached to new intra-communicator")
    parameter("newcomm", "COMMUNICATOR", direction="OUT", desc=r"new communicator")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_free")
    
    parameter(
        "comm",
        "COMMUNICATOR",
        direction="inout",
        desc=r"communicator to be destroyed",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_get_info")
    
    parameter("comm", "COMMUNICATOR", desc=r"communicator object")
    parameter("info_used", "INFO", direction="out", desc=r"new info object")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_test_inter")
    
    parameter("comm", "COMMUNICATOR", desc=r"communicator")
    parameter("flag", "LOGICAL", direction="out", desc=r"\mpicode{true} if \mpiarg{comm} is an inter-communicator")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_remote_size")
    
    parameter("comm", "COMMUNICATOR", desc=r"inter-communicator")
    parameter(
        "size",
        "COMM_SIZE",
        direction="out",
        desc=r"number of \MPI/ processes in the remote group of \mpiarg{comm}",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Group}%
\begin{mpi-binding}
    function_name("MPI_Comm_remote_group")
    ...
    parameter(
        ...
        "GROUP",
        direction="out",
        desc=r"remote group corresponding to \mpiarg{comm}",
    )
\end{mpi-binding}
...
\cdeclmainindex{MPI\_Comm}\cdeclmainindex{TYPE(MPI\_Comm)}
\begin{mpi-binding}
    function_name("MPI_Intercomm_create")
    
    parameter("local_comm", "COMMUNICATOR", desc=r"local intra-communicator")
    parameter(
        "local_leader",
        "RANK",
        desc=r"rank of local group leader in \mpiarg{local_comm}",
    )
    parameter(
        "peer_comm",
        "COMMUNICATOR",
        desc=r"``peer'' communicator; significant only at the \mpiarg{local_leader}",
    )
    parameter(
        "remote_leader",
        "RANK",
        desc=r"rank of remote group leader in \mpiarg{peer_comm}; significant only at the \mpiarg{local_leader}",
    )
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Intercomm_create_from_groups")
    parameter("local_group", "GROUP", desc=r"local group")
    parameter("local_leader", "RANK", desc=r"rank of local group leader in \mpiarg{local\_group}")
    parameter("remote_group", "GROUP", desc=r"remote group, significant only at \mpiarg{local\_leader}")
    parameter("remote_leader", "RANK", desc=r"rank of remote group leader in \mpiarg{remote\_group}, "
                                            r"significant only at \mpiarg{local\_leader}")
    parameter("stringtag", "STRING", constant=True, desc=r"unique identifier for this operation")
    parameter("info", "INFO", desc=r"info object")
    parameter("errhandler", "ERRHANDLER", desc=r"error handler to be attached to new inter-communicator")
    parameter("newintercomm", "COMMUNICATOR", direction="OUT", desc=r"new inter-communicator")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Intercomm_merge")
    
    parameter("intercomm", "COMMUNICATOR", desc=r"inter-communicator")
    parameter("high", "LOGICAL", desc=r"ordering of the local and remote groups in the new intra-communicator")
    parameter(
        "newintracomm",
        "COMMUNICATOR",
        direction="out",
        desc=r"new intra-communicator",
    )
\end{mpi-binding}
...
\begin{figure}
  \centerline{\includegraphics[width=4.0in]{figures/context-fig-1}}
...
\begin{figure}
  \centerline{\includegraphics[width=4.0in]{figures/context-fig-2}}
...
\begin{mpi-binding}
    ...
    parameter(
        ...
        desc=r"copy callback function for \mpiarg{comm_keyval}",
    )
    parameter(
        "comm_delete_attr_fn",
        "FUNCTION",
        func_type="MPI_Comm_delete_attr_function",
        desc=r"delete callback function for \mpiarg{comm_keyval}",
    )
    ...
    parameter(
        ...
        "KEYVAL",
        direction="out",
        desc=r"key value for future access",
    )
    parameter(
        "extra_state", "EXTRA_STATE", desc=r"extra state for callback function"
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    ...
    function_name("MPI_Comm_copy_attr_function", "COMM_COPY_ATTR_FUNCTION")
    
    parameter("oldcomm", "COMMUNICATOR")
    parameter("comm_keyval", "KEYVAL")
    parameter("extra_state", "EXTRA_STATE")
    parameter("attribute_val_in", "ATTRIBUTE_VAL")
    parameter("attribute_val_out", "ATTRIBUTE_VAL", direction="out")
...
\begin{mpi-binding}
    ...
    function_name("MPI_Comm_delete_attr_function", "COMM_DELETE_ATTR_FUNCTION")
    
    parameter("comm", "COMMUNICATOR")
    parameter("comm_keyval", "KEYVAL")
    parameter("attribute_val", "ATTRIBUTE_VAL", direction="out")
    parameter("extra_state", "EXTRA_STATE")
...
\begin{mpi-binding}
    render("mpi_comm_copy_attr_function", "f08")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_comm_delete_attr_function", "f08")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_comm_copy_attr_function", "f90")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_comm_delete_attr_function", "f90")
\end{mpi-binding}
...
\begin{mpi-binding}
    predefined_function("MPI_Comm_copy_attr_function")
    no_lis_binding()
    
    function_name("MPI_COMM_NULL_COPY_FN")
    
    parameter("oldcomm", "COMMUNICATOR")
    parameter("comm_keyval", "KEYVAL")
    parameter("extra_state", "EXTRA_STATE")
    parameter("attribute_val_in", "ATTRIBUTE_VAL")
    parameter("attribute_val_out", "ATTRIBUTE_VAL")
    parameter("flag", "LOGICAL", direction="out")
...
% Therefore, they are surrounded by a deleting macro.
\begin{mpi-binding}
    predefined_function("MPI_Comm_delete_attr_function")
    no_lis_binding()
    
    function_name("MPI_COMM_NULL_DELETE_FN")
    
    parameter("comm", "COMMUNICATOR")
    parameter("comm_keyval", "KEYVAL")
    parameter("attribute_val", "ATTRIBUTE_VAL")
    parameter("extra_state", "EXTRA_STATE")
...
\begin{mpi-binding}
    function_name("MPI_Comm_free_keyval")
    
    parameter("comm_keyval", "KEYVAL", direction="inout", desc=r"key value")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_set_attr")
    ...
    parameter("comm_keyval", "KEYVAL", desc=r"key value")
    parameter("attribute_val", "ATTRIBUTE_VAL", desc=r"attribute value")
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_get_attr")
    
    parameter(
        "comm",
        "COMMUNICATOR",
        desc=r"communicator to which the attribute is attached",
    )
    parameter("comm_keyval", "KEYVAL", desc=r"key value")
    parameter(
        "attribute_val",
    ...
    parameter(
        ...
        "LOGICAL",
        direction="out",
        desc=r"\mpicode{false} if no attribute is associated with the key",
    )
\end{mpi-binding}
...
\cdeclindex{MPI\_Comm}%
\begin{mpi-binding}
    function_name("MPI_Comm_delete_attr")
    
    parameter(
        "comm",
        "COMMUNICATOR",
        direction="lis:inout,param:in",
        desc=r"communicator from which the attribute is deleted",
    )
    parameter("comm_keyval", "KEYVAL", desc=r"key value")
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Win_create_keyval")
    
    parameter(
        "win_copy_attr_fn",
        "FUNCTION",
        func_type="MPI_Win_copy_attr_function",
        desc=r"copy callback function for \mpiarg{win_keyval}",
    )
    parameter(
        "win_delete_attr_fn",
        "FUNCTION",
        func_type="MPI_Win_delete_attr_function",
        desc=r"delete callback function for \mpiarg{win_keyval}",
    )
...
% Therefore, they are surrounded by a deleting macro.
\begin{mpi-binding}
    predefined_function("MPI_Win_copy_attr_function")
    no_lis_binding()
    
    function_name("MPI_WIN_NULL_COPY_FN")
    
    parameter("oldwin", "WINDOW")
    parameter("win_keyval", "KEYVAL")
    parameter("extra_state", "EXTRA_STATE")
    parameter("attribute_val_in", "ATTRIBUTE_VAL")
    parameter("attribute_val_out", "ATTRIBUTE_VAL")
    parameter("flag", "LOGICAL", direction="out")
...
% Therefore, they are surrounded by a deleting macro.
\begin{mpi-binding}
    predefined_function("MPI_Win_delete_attr_function")
    no_lis_binding()
    
    function_name("MPI_WIN_NULL_DELETE_FN")
...
\begin{mpi-binding}
    ...
    function_name("MPI_Win_copy_attr_function", "WIN_COPY_ATTR_FUNCTION")
...
\begin{mpi-binding}
    ...
    function_name("MPI_Win_delete_attr_function", "WIN_DELETE_ATTR_FUNCTION")
...
\begin{mpi-binding}
    render("mpi_win_copy_attr_function", "f08")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_win_delete_attr_function", "f08")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_win_copy_attr_function", "f90")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_win_delete_attr_function", "f90")
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Win_free_keyval")
    
    parameter("win_keyval", "KEYVAL", direction="inout", desc=r"key value")
\end{mpi-binding}

\cdeclindex{MPI\_Win}%
\begin{mpi-binding}
    function_name("MPI_Win_set_attr")
    
    parameter(
        "win",
        "WINDOW",
        direction="lis:inout,param:in",
        desc=r"window to which attribute will be attached",
    )
    
    parameter("win_keyval", "KEYVAL", desc=r"key value")
    parameter("attribute_val", "ATTRIBUTE_VAL", desc=r"attribute value")
\end{mpi-binding}

\cdeclindex{MPI\_Win}%
\begin{mpi-binding}
    function_name("MPI_Win_get_attr")
    
    parameter("win", "WINDOW", desc=r"window to which the attribute is attached")
    parameter("win_keyval", "KEYVAL", desc=r"key value")
    parameter(
        "attribute_val",
        "ATTRIBUTE_VAL",
...
\cdeclindex{MPI\_Win}%
\begin{mpi-binding}
    function_name("MPI_Win_delete_attr")
    
    parameter(
        "win",
        "WINDOW",
        direction="lis:inout,param:in",
        desc=r"window from which the attribute is deleted",
    )
    parameter("win_keyval", "KEYVAL", desc=r"key value")
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Type_create_keyval")
    
    parameter(
        "type_copy_attr_fn",
        "FUNCTION",
        func_type="MPI_Type_copy_attr_function",
        desc=r"copy callback function for \mpiarg{type_keyval}",
    )
    parameter(
        "type_delete_attr_fn",
        "FUNCTION",
        func_type="MPI_Type_delete_attr_function",
        desc=r"delete callback function for \mpiarg{type_keyval}",
    )
...
% Therefore, they are surrounded by a deleting macro.
\begin{mpi-binding}
    predefined_function("MPI_Type_copy_attr_function")
    no_lis_binding()
    
    function_name("MPI_TYPE_NULL_COPY_FN")
    
    parameter("oldtype", "DATATYPE")
    parameter("type_keyval", "KEYVAL")
    parameter("extra_state", "EXTRA_STATE")
    parameter("attribute_val_in", "ATTRIBUTE_VAL")
    parameter("attribute_val_out", "ATTRIBUTE_VAL")
    parameter("flag", "LOGICAL", direction="out")
...
% Therefore, they are surrounded by a deleting macro.
\begin{mpi-binding}
    predefined_function("MPI_Type_delete_attr_function")
    no_lis_binding()
    
    function_name("MPI_TYPE_NULL_DELETE_FN")
...
\begin{mpi-binding}
    ...
    function_name("MPI_Type_copy_attr_function", "TYPE_COPY_ATTR_FUNCTION")
...
\begin{mpi-binding}
    ...
    function_name("MPI_Type_delete_attr_function", "TYPE_DELETE_ATTR_FUNCTION")
...
\begin{mpi-binding}
    render("mpi_type_copy_attr_function", "f08")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_type_delete_attr_function", "f08")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_type_copy_attr_function", "f90")
\end{mpi-binding}
...
\begin{mpi-binding}
    render("mpi_type_delete_attr_function", "f90")
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Type_free_keyval")
    
    parameter("type_keyval", "KEYVAL", direction="inout", desc=r"key value")
\end{mpi-binding}

\begin{mpi-binding}
    function_name("MPI_Type_set_attr")
    
    parameter(
        "datatype",
        "DATATYPE",
        direction="lis:inout,param:in",
        desc=r"datatype to which attribute will be attached",
    )
    
    parameter("type_keyval", "KEYVAL", desc=r"key value")
    parameter("attribute_val", "ATTRIBUTE_VAL", desc=r"attribute value")
\end{mpi-binding}

\begin{mpi-binding}
    function_name("MPI_Type_get_attr")
    
    parameter(
        "datatype",
        "DATATYPE",
        desc=r"datatype to which the attribute is attached",
    )
    parameter("type_keyval", "KEYVAL", desc=r"key value")
    parameter(
        "attribute_val",
...
\begin{mpi-binding}
    function_name("MPI_Type_delete_attr")
    
    parameter(
        "datatype",
        "DATATYPE",
        direction="lis:inout,param:in",
        desc=r"datatype from which the attribute is deleted",
    )
    parameter("type_keyval", "KEYVAL", desc=r"key value")
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Comm_set_name")
    
    parameter(
        "comm",
        "COMMUNICATOR",
        direction="lis:inout,param:in",
        desc=r"communicator whose identifier is to be set",
    )
    parameter(
        "comm_name",
        "STRING",
        constant=True,
        desc=r"the character string that is remembered as the name",
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Comm_get_name")
    
    parameter(
        "comm", "COMMUNICATOR", desc=r"communicator whose name is to be returned"
    )
    parameter(
        "comm_name",
        "STRING",
        direction="out",
        length="MPI_MAX_OBJECT_NAME",
        desc=(
            "the name previously stored on the communicator, or an empty "
            "string if no such name exists"
        ),
    )
    parameter(
        "resultlen",
        "STRING_LENGTH",
        direction="out",
        desc=r"length of returned name",
    )
\end{mpi-binding}
...
\begin{mpi-binding}
    function_name("MPI_Type_set_name")
    
    parameter(
        "datatype",
        "DATATYPE",
        direction="lis:inout,param:in",
        desc=r"datatype whose identifier is to be set",
    )
    parameter(
        "type_name",
        "STRING",
...
\begin{mpi-binding}
    function_name("MPI_Type_get_name")
    
    parameter(
        "datatype", "DATATYPE", desc=r"datatype whose name is to be returned"
    )
    parameter(
        "type_name",
        "STRING",
        ...
        length="MPI_MAX_OBJECT_NAME",
        desc=(
            "the name previously stored on the datatype, or an empty string "
...
\begin{mpi-binding}
    function_name("MPI_Win_set_name")
    
    parameter(
        "win",
        "WINDOW",
        direction="lis:inout,param:in",
        desc=r"window whose identifier is to be set",
    )
    parameter(
        "win_name",
        "STRING",
...
\begin{mpi-binding}
    function_name("MPI_Win_get_name")
    
    parameter("win", "WINDOW", desc=r"window whose name is to be returned")
    parameter(
        "win_name",
        "STRING",
        direction="out",
        length="MPI_MAX_OBJECT_NAME",
        desc=(
            "the name previously stored on the window, or an empty string "
            "if no such name exists"
        ),
    )
    parameter(
