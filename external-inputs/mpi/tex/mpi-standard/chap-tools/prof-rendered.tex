...
\begin{funcdef}{MPI\_PCONTROL}{(\mbox{level}, \mbox{\ldots})}
\funcarg{\IN}{level}{Profiling level (integer)}
\end{funcdef}
\mpibindmain{MPI\_Pcontrol}{(\gb{}const~int~level, \ldots)}
\mpifnewbindmain{MPI\_Pcontrol}{(level) \fargs \fnfinalarg{}INTEGER, INTENT(IN)\ ::\ \mbox{level}}
\mpifbindmain{MPI\_PCONTROL}{(LEVEL) \fargs \fnfinalarg{}INTEGER \mbox{LEVEL}}
