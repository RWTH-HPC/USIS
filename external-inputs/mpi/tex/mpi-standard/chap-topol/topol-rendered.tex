...
\begin{funcdef}{MPI\_CART\_CREATE}{(\mbox{comm\_old}, \mbox{ndims}, \mbox{dims}, \mbox{periods}, \mbox{reorder}, \mbox{comm\_cart})}
...
\funcarg{\IN}{periods}{logical array of size \mpiarg{ndims} specifying whether the grid is periodic (\mpicode{true}) or not (\mpicode{false}) in each dimension}
\funcarg{\IN}{reorder}{ranks may be reordered (\mpicode{true}) or not (\mpicode{false}) (logical)}
\funcarg{\OUT}{comm\_cart}{new communicator with associated Cartesian topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_create}{(\gb{}MPI\_Comm~comm\_old, int~ndims, const~int~dims[], const~int~periods[], int~reorder, MPI\_Comm~*comm\_cart)}
\mpifnewbindmain{MPI\_Cart\_create}{(comm\_old, ndims, dims, periods, reorder, comm\_cart, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm\_old}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{ndims}, \mbox{dims(ndims)}\fnarg{}LOGICAL, INTENT(IN)\ ::\ \mbox{periods(ndims)}, \mbox{reorder}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{comm\_cart}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_CREATE}{(COMM\_OLD, NDIMS, DIMS, PERIODS, REORDER, COMM\_CART, IERROR) \fargs INTEGER \mbox{COMM\_OLD}, \mbox{NDIMS}, \mbox{DIMS(*)}, \mbox{COMM\_CART}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{PERIODS(*)}, \mbox{REORDER}}
...
\begin{funcdef}{MPI\_DIMS\_CREATE}{(\mbox{nnodes}, \mbox{ndims}, \mbox{dims})}
\funcarg{\IN}{nnodes}{number of nodes in a grid (integer)}
\funcarg{\IN}{ndims}{number of Cartesian dimensions (integer)}
\funcarg{\INOUT}{dims}{integer array of size \mpiarg{ndims} specifying the number of nodes in each dimension}
\end{funcdef}
\mpibindmain{MPI\_Dims\_create}{(\gb{}int~nnodes, int~ndims, int~dims[])}
\mpifnewbindmain{MPI\_Dims\_create}{(nnodes, ndims, dims, ierror) \fargs INTEGER, INTENT(IN)\ ::\ \mbox{nnodes}, \mbox{ndims}\fnarg{}INTEGER, INTENT(INOUT)\ ::\ \mbox{dims(ndims)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_DIMS\_CREATE}{(NNODES, NDIMS, DIMS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{NNODES}, \mbox{NDIMS}, \mbox{DIMS(*)}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_GRAPH\_CREATE}{(\mbox{comm\_old}, \mbox{nnodes}, \mbox{index}, \mbox{edges}, \mbox{reorder}, \mbox{comm\_graph})}
\funcarg{\IN}{comm\_old}{input communicator (handle)}
...
\funcarg{\IN}{reorder}{ranks may be reordered (\mpicode{true}) or not (\mpicode{false}) (logical)}
\funcarg{\OUT}{comm\_graph}{new communicator with associated graph topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Graph\_create}{(\gb{}MPI\_Comm~comm\_old, int~nnodes, const~int~index[], const~int~edges[], int~reorder, MPI\_Comm~*comm\_graph)}
\mpifnewbindmain{MPI\_Graph\_create}{(comm\_old, nnodes, index, edges, reorder, comm\_graph, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm\_old}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{nnodes}, \mbox{index(nnodes)}, \mbox{edges(*)}\fnarg{}LOGICAL, INTENT(IN)\ ::\ \mbox{reorder}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{comm\_graph}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_GRAPH\_CREATE}{(COMM\_OLD, NNODES, INDEX, EDGES, REORDER, COMM\_GRAPH, IERROR) \fargs INTEGER \mbox{COMM\_OLD}, \mbox{NNODES}, \mbox{INDEX(*)}, \mbox{EDGES(*)}, \mbox{COMM\_GRAPH}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{REORDER}}
...
\begin{funcdef}{MPI\_DIST\_GRAPH\_CREATE\_ADJACENT}{(\mbox{comm\_old}, \mbox{indegree}, \mbox{sources}, \mbox{sourceweights}, \mbox{outdegree}, \mbox{destinations}, \mbox{destweights}, \mbox{info}, \mbox{reorder}, \mbox{comm\_dist\_graph})}
\funcarg{\IN}{comm\_old}{input communicator (handle)}
\funcarg{\IN}{indegree}{size of \mpiarg{sources} and \mpiarg{sourceweights} arrays (nonnegative integer)}
\funcarg{\IN}{sources}{ranks of \MPI/ processes for which the calling process is a destination (array of nonnegative integers)}
\funcarg{\IN}{sourceweights}{weights of the edges into the calling \MPI/ process (array of nonnegative integers)}
\funcarg{\IN}{outdegree}{size of \mpiarg{destinations} and \mpiarg{destweights} arrays (nonnegative integer)}
\funcarg{\IN}{destinations}{ranks of \MPI/ processes for which the calling \MPI/ process is a source (array of nonnegative integers)}
\funcarg{\IN}{destweights}{weights of the edges out of the calling \MPI/ process (array of nonnegative integers)}
\funcarg{\IN}{info}{hints on optimization and interpretation of weights (handle)}
\funcarg{\IN}{reorder}{ranks may be reordered (\mpicode{true}) or not~(\mbox{\mpicode{false}}) (logical)}
\funcarg{\OUT}{comm\_dist\_graph}{new communicator with associated distributed graph topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Dist\_graph\_create\_adjacent}{(\gb{}MPI\_Comm~comm\_old, int~indegree, const~int~sources[], const~int~sourceweights[], int~outdegree, const~int~destinations[], const~int~destweights[], MPI\_Info~info, int~reorder, MPI\_Comm~*comm\_dist\_graph)}
\mpifnewbindmain{MPI\_Dist\_graph\_create\_adjacent}{(comm\_old, indegree, sources, sourceweights, outdegree, destinations, destweights, info, reorder, comm\_dist\_graph, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm\_old}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{indegree}, \mbox{sources(indegree)}, \mbox{sourceweights(*)}, \mbox{outdegree}, \mbox{destinations(outdegree)}, \mbox{destweights(*)}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}LOGICAL, INTENT(IN)\ ::\ \mbox{reorder}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{comm\_dist\_graph}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_DIST\_GRAPH\_CREATE\_ADJACENT}{(COMM\_OLD, INDEGREE, SOURCES, SOURCEWEIGHTS, OUTDEGREE, DESTINATIONS, DESTWEIGHTS, INFO, REORDER, COMM\_DIST\_GRAPH, IERROR) \fargs INTEGER \mbox{COMM\_OLD}, \mbox{INDEGREE}, \mbox{SOURCES(*)}, \mbox{SOURCEWEIGHTS(*)}, \mbox{OUTDEGREE}, \mbox{DESTINATIONS(*)}, \mbox{DESTWEIGHTS(*)}, \mbox{INFO}, \mbox{COMM\_DIST\_GRAPH}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{REORDER}}
...
\begin{funcdef}{MPI\_DIST\_GRAPH\_CREATE}{(\mbox{comm\_old}, \mbox{n}, \mbox{sources}, \mbox{degrees}, \mbox{destinations}, \mbox{weights}, \mbox{info}, \mbox{reorder}, \mbox{comm\_dist\_graph})}
\funcarg{\IN}{comm\_old}{input communicator (handle)}
\funcarg{\IN}{n}{number of source nodes for which this \MPI/ process specifies edges (nonnegative integer)}
\funcarg{\IN}{sources}{array containing the \mpiarg{n} source nodes for which this \MPI/ process specifies edges (array of nonnegative integers)}
\funcarg{\IN}{degrees}{array specifying the number of destinations for each source node in the source node array (array of nonnegative integers)}
\funcarg{\IN}{destinations}{destination nodes for the source nodes in the source node array (array of nonnegative integers)}
\funcarg{\IN}{weights}{weights for source to destination edges (array of nonnegative integers)}
\funcarg{\IN}{info}{hints on optimization and interpretation of weights (handle)}
\funcarg{\IN}{reorder}{ranks may be reordered (\mpicode{true}) or not~(\mbox{\mpicode{false}}) (logical)}
\funcarg{\OUT}{comm\_dist\_graph}{new communicator with associated distributed graph topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Dist\_graph\_create}{(\gb{}MPI\_Comm~comm\_old, int~n, const~int~sources[], const~int~degrees[], const~int~destinations[], const~int~weights[], MPI\_Info~info, int~reorder, MPI\_Comm~*comm\_dist\_graph)}
\mpifnewbindmain{MPI\_Dist\_graph\_create}{(comm\_old, n, sources, degrees, destinations, weights, info, reorder, comm\_dist\_graph, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm\_old}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{n}, \mbox{sources(n)}, \mbox{degrees(n)}, \mbox{destinations(*)}, \mbox{weights(*)}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}LOGICAL, INTENT(IN)\ ::\ \mbox{reorder}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{comm\_dist\_graph}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_DIST\_GRAPH\_CREATE}{(COMM\_OLD, N, SOURCES, DEGREES, DESTINATIONS, WEIGHTS, INFO, REORDER, COMM\_DIST\_GRAPH, IERROR) \fargs INTEGER \mbox{COMM\_OLD}, \mbox{N}, \mbox{SOURCES(*)}, \mbox{DEGREES(*)}, \mbox{DESTINATIONS(*)}, \mbox{WEIGHTS(*)}, \mbox{INFO}, \mbox{COMM\_DIST\_GRAPH}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{REORDER}}
...
\begin{funcdef}{MPI\_TOPO\_TEST}{(\mbox{comm}, \mbox{status})}
\funcarg{\IN}{comm}{communicator (handle)}
\funcarg{\OUT}{status}{topology type of communicator \mpiarg{comm} (state)}
\end{funcdef}
\mpibindmain{MPI\_Topo\_test}{(\gb{}MPI\_Comm~comm, int~*status)}
\mpifnewbindmain{MPI\_Topo\_test}{(comm, status, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{status}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_TOPO\_TEST}{(COMM, STATUS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{STATUS}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_GRAPHDIMS\_GET}{(\mbox{comm}, \mbox{nnodes}, \mbox{nedges})}
\funcarg{\IN}{comm}{communicator with associated graph topology (handle)}
\funcarg{\OUT}{nnodes}{number of nodes in graph (same as number of \MPI/ processes in the group of \mpiarg{comm}) (integer)}
\funcarg{\OUT}{nedges}{number of edges in graph (integer)}
\end{funcdef}
\mpibindmain{MPI\_Graphdims\_get}{(\gb{}MPI\_Comm~comm, int~*nnodes, int~*nedges)}
\mpifnewbindmain{MPI\_Graphdims\_get}{(comm, nnodes, nedges, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{nnodes}, \mbox{nedges}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_GRAPHDIMS\_GET}{(COMM, NNODES, NEDGES, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{NNODES}, \mbox{NEDGES}, \mbox{IERROR}}
\begin{sloppy}
...
\begin{funcdef}{MPI\_GRAPH\_GET}{(\mbox{comm}, \mbox{maxindex}, \mbox{maxedges}, \mbox{index}, \mbox{edges})}
\funcarg{\IN}{comm}{communicator with associated graph topology (handle)}
...
\funcarg{\OUT}{index}{array of integers containing the graph structure (for details see the definition of \mpifunc{MPI\_GRAPH\_CREATE})}
\funcarg{\OUT}{edges}{array of integers containing the graph structure}
\end{funcdef}
\mpibindmain{MPI\_Graph\_get}{(\gb{}MPI\_Comm~comm, int~maxindex, int~maxedges, int~index[], int~edges[])}
\mpifnewbindmain{MPI\_Graph\_get}{(comm, maxindex, maxedges, index, edges, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{maxindex}, \mbox{maxedges}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{index(maxindex)}, \mbox{edges(maxedges)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_GRAPH\_GET}{(COMM, MAXINDEX, MAXEDGES, INDEX, EDGES, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{MAXINDEX}, \mbox{MAXEDGES}, \mbox{INDEX(*)}, \mbox{EDGES(*)}, \mbox{IERROR}}

\begin{funcdef}{MPI\_CARTDIM\_GET}{(\mbox{comm}, \mbox{ndims})}
\funcarg{\IN}{comm}{communicator with associated Cartesian topology (handle)}
\funcarg{\OUT}{ndims}{number of dimensions of the Cartesian structure (integer)}
\end{funcdef}
\mpibindmain{MPI\_Cartdim\_get}{(\gb{}MPI\_Comm~comm, int~*ndims)}
\mpifnewbindmain{MPI\_Cartdim\_get}{(comm, ndims, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{ndims}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CARTDIM\_GET}{(COMM, NDIMS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{NDIMS}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_CART\_GET}{(\mbox{comm}, \mbox{maxdims}, \mbox{dims}, \mbox{periods}, \mbox{coords})}
\funcarg{\IN}{comm}{communicator with associated Cartesian topology (handle)}
\funcarg{\IN}{maxdims}{length of vectors \mpiarg{dims}, \mpiarg{periods}, and \mpiarg{coords} in the calling program (integer)}
\funcarg{\OUT}{dims}{number of \MPI/ processes for each Cartesian dimension (array of integers)}
\funcarg{\OUT}{periods}{periodicity (\mpicode{true}/\mpicode{false}) for each Cartesian dimension (array of logicals)}
\funcarg{\OUT}{coords}{coordinates of calling \MPI/ process in Cartesian structure (array of integers)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_get}{(\gb{}MPI\_Comm~comm, int~maxdims, int~dims[], int~periods[], int~coords[])}
\mpifnewbindmain{MPI\_Cart\_get}{(comm, maxdims, dims, periods, coords, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{maxdims}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{dims(maxdims)}, \mbox{coords(maxdims)}\fnarg{}LOGICAL, INTENT(OUT)\ ::\ \mbox{periods(maxdims)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_GET}{(COMM, MAXDIMS, DIMS, PERIODS, COORDS, IERROR) \fargs INTEGER \mbox{COMM}, \mbox{MAXDIMS}, \mbox{DIMS(*)}, \mbox{COORDS(*)}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{PERIODS(*)}}
...
\begin{funcdef}{MPI\_CART\_RANK}{(\mbox{comm}, \mbox{coords}, \mbox{rank})}
\funcarg{\IN}{comm}{communicator with associated Cartesian topology (handle)}
\funcarg{\IN}{coords}{integer array (of size \mpiarg{ndims}) specifying the Cartesian coordinates of an \MPI/ process}
\funcarg{\OUT}{rank}{rank of specified \MPI/ process within group of \mpiarg{comm} (integer)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_rank}{(\gb{}MPI\_Comm~comm, const~int~coords[], int~*rank)}
\mpifnewbindmain{MPI\_Cart\_rank}{(comm, coords, rank, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{coords(*)}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{rank}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_RANK}{(COMM, COORDS, RANK, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{COORDS(*)}, \mbox{RANK}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_CART\_COORDS}{(\mbox{comm}, \mbox{rank}, \mbox{maxdims}, \mbox{coords})}
\funcarg{\IN}{comm}{communicator with associated Cartesian topology (handle)}
\funcarg{\IN}{rank}{rank of an \MPI/ process within group of \mpiarg{comm} (integer)}
\funcarg{\IN}{maxdims}{length of vector  \mpiarg{coords} in the calling program (integer)}
\funcarg{\OUT}{coords}{coordinates of the \MPI/ process with the rank \mpiarg{rank} in Cartesian structure (array of integers)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_coords}{(\gb{}MPI\_Comm~comm, int~rank, int~maxdims, int~coords[])}
\mpifnewbindmain{MPI\_Cart\_coords}{(comm, rank, maxdims, coords, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{rank}, \mbox{maxdims}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{coords(maxdims)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_COORDS}{(COMM, RANK, MAXDIMS, COORDS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{RANK}, \mbox{MAXDIMS}, \mbox{COORDS(*)}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_GRAPH\_NEIGHBORS\_COUNT}{(\mbox{comm}, \mbox{rank}, \mbox{nneighbors})}
\funcarg{\IN}{comm}{communicator with associated graph topology (handle)}
\funcarg{\IN}{rank}{rank of \MPI/ process in group of \mpiarg{comm} (integer)}
\funcarg{\OUT}{nneighbors}{number of neighbors of specified \MPI/ process (integer)}
\end{funcdef}
\mpibindmain{MPI\_Graph\_neighbors\_count}{(\gb{}MPI\_Comm~comm, int~rank, int~*nneighbors)}
\mpifnewbindmain{MPI\_Graph\_neighbors\_count}{(comm, rank, nneighbors, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{rank}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{nneighbors}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_GRAPH\_NEIGHBORS\_COUNT}{(COMM, RANK, NNEIGHBORS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{RANK}, \mbox{NNEIGHBORS}, \mbox{IERROR}}

\begin{funcdef}{MPI\_GRAPH\_NEIGHBORS}{(\mbox{comm}, \mbox{rank}, \mbox{maxneighbors}, \mbox{neighbors})}
\funcarg{\IN}{comm}{communicator with associated graph topology (handle)}
\funcarg{\IN}{rank}{rank of \MPI/ process in group of \mpiarg{comm} (integer)}
\funcarg{\IN}{maxneighbors}{size of array \mpiarg{neighbors} (integer)}
\funcarg{\OUT}{neighbors}{ranks of \MPI/ processes that are neighbors to specified \MPI/ process (array of integers)}
\end{funcdef}
\mpibindmain{MPI\_Graph\_neighbors}{(\gb{}MPI\_Comm~comm, int~rank, int~maxneighbors, int~neighbors[])}
\mpifnewbindmain{MPI\_Graph\_neighbors}{(comm, rank, maxneighbors, neighbors, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{rank}, \mbox{maxneighbors}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{neighbors(maxneighbors)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_GRAPH\_NEIGHBORS}{(COMM, RANK, MAXNEIGHBORS, NEIGHBORS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{RANK}, \mbox{MAXNEIGHBORS}, \mbox{NEIGHBORS(*)}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_DIST\_GRAPH\_NEIGHBORS\_COUNT}{(\mbox{comm}, \mbox{indegree}, \mbox{outdegree}, \mbox{weighted})}
\funcarg{\IN}{comm}{communicator with associated distributed graph topology (handle)}
\funcarg{\OUT}{indegree}{number of edges into this \MPI/ process (nonnegative integer)}
\funcarg{\OUT}{outdegree}{number of edges out of this \MPI/ process (nonnegative integer)}
\funcarg{\OUT}{weighted}{\mpicode{false} if \mpiconst{MPI\_UNWEIGHTED} was supplied during creation, \mpicode{true} otherwise (logical)}
\end{funcdef}
\mpibindmain{MPI\_Dist\_graph\_neighbors\_count}{(\gb{}MPI\_Comm~comm, int~*indegree, int~*outdegree, int~*weighted)}
\mpifnewbindmain{MPI\_Dist\_graph\_neighbors\_count}{(comm, indegree, outdegree, weighted, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{indegree}, \mbox{outdegree}\fnarg{}LOGICAL, INTENT(OUT)\ ::\ \mbox{weighted}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_DIST\_GRAPH\_NEIGHBORS\_COUNT}{(COMM, INDEGREE, OUTDEGREE, WEIGHTED, IERROR) \fargs INTEGER \mbox{COMM}, \mbox{INDEGREE}, \mbox{OUTDEGREE}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{WEIGHTED}}

\begin{funcdef}{MPI\_DIST\_GRAPH\_NEIGHBORS}{(\mbox{comm}, \mbox{maxindegree}, \mbox{sources}, \mbox{sourceweights}, \mbox{maxoutdegree}, \mbox{destinations}, \mbox{destweights})}
\funcarg{\IN}{comm}{communicator with associated distributed graph topology (handle)}
\funcarg{\IN}{maxindegree}{size of sources and sourceweights arrays (nonnegative integer)}
\funcarg{\OUT}{sources}{ranks of \MPI/ processes for which the calling \MPI/ process is a destination (array of nonnegative integers)}
\funcarg{\OUT}{sourceweights}{weights of the edges into the calling \MPI/ process (array of nonnegative integers)}
\funcarg{\IN}{maxoutdegree}{size of destinations and destweights arrays (nonnegative integer)}
\funcarg{\OUT}{destinations}{ranks of \MPI/ processes for which the calling \MPI/ process is a source (array of nonnegative integers)}
\funcarg{\OUT}{destweights}{weights of the edges out of the calling \MPI/ process (array of nonnegative integers)}
\end{funcdef}
\mpibindmain{MPI\_Dist\_graph\_neighbors}{(\gb{}MPI\_Comm~comm, int~maxindegree, int~sources[], int~sourceweights[], int~maxoutdegree, int~destinations[], int~destweights[])}
\mpifnewbindmain{MPI\_Dist\_graph\_neighbors}{(comm, maxindegree, sources, sourceweights, maxoutdegree, destinations, destweights, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{maxindegree}, \mbox{maxoutdegree}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{sources(maxindegree)}, \mbox{destinations(maxoutdegree)}\fnarg{}INTEGER\ ::\ \mbox{sourceweights(*)}, \mbox{destweights(*)}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_DIST\_GRAPH\_NEIGHBORS}{(COMM, MAXINDEGREE, SOURCES, SOURCEWEIGHTS, MAXOUTDEGREE, DESTINATIONS, DESTWEIGHTS, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{MAXINDEGREE}, \mbox{SOURCES(*)}, \mbox{SOURCEWEIGHTS(*)}, \mbox{MAXOUTDEGREE}, \mbox{DESTINATIONS(*)}, \mbox{DESTWEIGHTS(*)}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_CART\_SHIFT}{(\mbox{comm}, \mbox{direction}, \mbox{disp}, \mbox{rank\_source}, \mbox{rank\_dest})}
\funcarg{\IN}{comm}{communicator with associated Cartesian topology (handle)}
...
\funcarg{\OUT}{rank\_source}{rank of source \MPI/ process (integer)}
\funcarg{\OUT}{rank\_dest}{rank of destination \MPI/ process (integer)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_shift}{(\gb{}MPI\_Comm~comm, int~direction, int~disp, int~*rank\_source, int~*rank\_dest)}
\mpifnewbindmain{MPI\_Cart\_shift}{(comm, direction, disp, rank\_source, rank\_dest, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{direction}, \mbox{disp}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{rank\_source}, \mbox{rank\_dest}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_SHIFT}{(COMM, DIRECTION, DISP, RANK\_SOURCE, RANK\_DEST, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{DIRECTION}, \mbox{DISP}, \mbox{RANK\_SOURCE}, \mbox{RANK\_DEST}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_CART\_SUB}{(\mbox{comm}, \mbox{remain\_dims}, \mbox{newcomm})}
\funcarg{\IN}{comm}{communicator with associated Cartesian topology (handle)}
\funcarg{\IN}{remain\_dims}{the \mpicode{i}-th entry of \mpiarg{remain\_dims} specifies whether the \mpicode{i}-th dimension is kept in the subgrid (\mpicode{true}) or is dropped (\mpicode{false}) (array of logicals)}
\funcarg{\OUT}{newcomm}{new communicator with associated Cartesian topology containing the subgrid that includes the calling \MPI/ process (handle)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_sub}{(\gb{}MPI\_Comm~comm, const~int~remain\_dims[], MPI\_Comm~*newcomm)}
\mpifnewbindmain{MPI\_Cart\_sub}{(comm, remain\_dims, newcomm, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}LOGICAL, INTENT(IN)\ ::\ \mbox{remain\_dims(*)}\fnarg{}TYPE(MPI\_Comm), INTENT(OUT)\ ::\ \mbox{newcomm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_SUB}{(COMM, REMAIN\_DIMS, NEWCOMM, IERROR) \fargs INTEGER \mbox{COMM}, \mbox{NEWCOMM}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{REMAIN\_DIMS(*)}}
...
\begin{funcdef}{MPI\_CART\_MAP}{(\mbox{comm}, \mbox{ndims}, \mbox{dims}, \mbox{periods}, \mbox{newrank})}
\funcarg{\IN}{comm}{input communicator (handle)}
\funcarg{\IN}{ndims}{number of dimensions of Cartesian structure (integer)}
\funcarg{\IN}{dims}{integer array of size \mpiarg{ndims} specifying the number of processes in each coordinate direction}
\funcarg{\IN}{periods}{logical array of size \mpiarg{ndims} specifying the periodicity specification in each coordinate direction}
\funcarg{\OUT}{newrank}{reordered rank of the calling \MPI/ process; \mpiconst{MPI\_UNDEFINED} if calling \MPI/ process does not belong to grid (integer)}
\end{funcdef}
\mpibindmain{MPI\_Cart\_map}{(\gb{}MPI\_Comm~comm, int~ndims, const~int~dims[], const~int~periods[], int~*newrank)}
\mpifnewbindmain{MPI\_Cart\_map}{(comm, ndims, dims, periods, newrank, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{ndims}, \mbox{dims(ndims)}\fnarg{}LOGICAL, INTENT(IN)\ ::\ \mbox{periods(ndims)}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{newrank}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_CART\_MAP}{(COMM, NDIMS, DIMS, PERIODS, NEWRANK, IERROR) \fargs INTEGER \mbox{COMM}, \mbox{NDIMS}, \mbox{DIMS(*)}, \mbox{NEWRANK}, \mbox{IERROR}\fnfinalarg{}LOGICAL \mbox{PERIODS(*)}}
...
\begin{funcdef}{MPI\_GRAPH\_MAP}{(\mbox{comm}, \mbox{nnodes}, \mbox{index}, \mbox{edges}, \mbox{newrank})}
\funcarg{\IN}{comm}{input communicator (handle)}
\funcarg{\IN}{nnodes}{number of graph nodes (integer)}
\funcarg{\IN}{index}{integer array specifying the graph structure (for details see the definition of \mpifunc{MPI\_GRAPH\_CREATE})}
\funcarg{\IN}{edges}{integer array specifying the graph structure}
\funcarg{\OUT}{newrank}{reordered rank of the calling \MPI/ process; \mpiconst{MPI\_UNDEFINED} if the calling \MPI/ process does not belong to graph (integer)}
\end{funcdef}
\mpibindmain{MPI\_Graph\_map}{(\gb{}MPI\_Comm~comm, int~nnodes, const~int~index[], const~int~edges[], int~*newrank)}
\mpifnewbindmain{MPI\_Graph\_map}{(comm, nnodes, index, edges, newrank, ierror) \fargs TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{nnodes}, \mbox{index(nnodes)}, \mbox{edges(*)}\fnarg{}INTEGER, INTENT(OUT)\ ::\ \mbox{newrank}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_GRAPH\_MAP}{(COMM, NNODES, INDEX, EDGES, NEWRANK, IERROR) \fargs \fnfinalarg{}INTEGER \mbox{COMM}, \mbox{NNODES}, \mbox{INDEX(*)}, \mbox{EDGES(*)}, \mbox{NEWRANK}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLGATHER}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcount}, \mbox{recvtype}, \mbox{comm})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
\funcarg{\IN}{recvcount}{number of elements received from each neighbor (nonnegative integer)}
\funcarg{\IN}{recvtype}{datatype of receive buffer elements (handle)}
\funcarg{\IN}{comm}{communicator with associated virtual topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_allgather}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, int~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\MPIbindindex{MPI\_Neighbor\_allgather\_c}
\mpibindmain{MPI\_Neighbor\_allgather\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, MPI\_Count~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\mpifnewbindmain{MPI\_Neighbor\_allgather}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
...
\mpifbindmain{MPI\_NEIGHBOR\_ALLGATHER}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, COMM, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNT}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLGATHERV}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{displs}, \mbox{recvtype}, \mbox{comm})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
\funcarg{\IN}{recvcounts}{nonnegative integer array (of length indegree) containing the number of elements that are received from each neighbor}
\funcarg{\IN}{displs}{integer array (of length indegree). Entry \mpicode{i} specifies the displacement (relative to \mpiarg{recvbuf}) at which to place the incoming data from neighbor \mpicode{i}}
\funcarg{\IN}{recvtype}{datatype of receive buffer elements (handle)}
\funcarg{\IN}{comm}{communicator with associated virtual topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_allgatherv}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, const~int~recvcounts[], const~int~displs[], MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\MPIbindindex{MPI\_Neighbor\_allgatherv\_c}
\mpibindmain{MPI\_Neighbor\_allgatherv\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~displs[], MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\mpifnewbindmain{MPI\_Neighbor\_allgatherv}{(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcounts(*)}, \mbox{displs(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_allgatherv}{(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcounts(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN)\ ::\ \mbox{displs(*)}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLGATHERV}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNTS, DISPLS, RECVTYPE, COMM, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNTS(*)}, \mbox{DISPLS(*)}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLTOALL}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcount}, \mbox{recvtype}, \mbox{comm})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
...
\funcarg{\IN}{comm}{communicator with associated virtual topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_alltoall}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, int~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\MPIbindindex{MPI\_Neighbor\_alltoall\_c}
\mpibindmain{MPI\_Neighbor\_alltoall\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, MPI\_Count~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\mpifnewbindmain{MPI\_Neighbor\_alltoall}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_alltoall}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLTOALL}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, COMM, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNT}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLTOALLV}{(\mbox{sendbuf}, \mbox{sendcounts}, \mbox{sdispls}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{rdispls}, \mbox{recvtype}, \mbox{comm})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcounts}{nonnegative integer array (of length outdegree) specifying the number of elements to send to each neighbor}
\funcarg{\IN}{sdispls}{integer array (of length outdegree). Entry \mpicode{j} specifies the displacement (relative to \mpiarg{sendbuf}) from which to send the outgoing data to neighbor \mpicode{j}}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
\funcarg{\IN}{recvcounts}{nonnegative integer array (of length indegree) specifying the number of elements that are received from each neighbor}
\funcarg{\IN}{rdispls}{integer array (of length indegree). Entry \mpicode{i} specifies the displacement (relative to \mpiarg{recvbuf}) at which to place the incoming data from neighbor \mpicode{i}}
\funcarg{\IN}{recvtype}{datatype of receive buffer elements (handle)}
\funcarg{\IN}{comm}{communicator with associated virtual topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_alltoallv}{(\gb{}const~void~*sendbuf, const~int~sendcounts[], const~int~sdispls[], MPI\_Datatype~sendtype, void~*recvbuf, const~int~recvcounts[], const~int~rdispls[], MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\MPIbindindex{MPI\_Neighbor\_alltoallv\_c}
\mpibindmain{MPI\_Neighbor\_alltoallv\_c}{(\gb{}const~void~*sendbuf, const~MPI\_Count~sendcounts[], const~MPI\_Aint~sdispls[], MPI\_Datatype~sendtype, void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~rdispls[], MPI\_Datatype~recvtype, MPI\_Comm~comm)}
\mpifnewbindmain{MPI\_Neighbor\_alltoallv}{(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype, comm, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcounts(*)}, \mbox{sdispls(*)}, \mbox{recvcounts(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_alltoallv}{(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype, comm, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN)\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLTOALLV}{(SENDBUF, SENDCOUNTS, SDISPLS, SENDTYPE, RECVBUF, RECVCOUNTS, RDISPLS, RECVTYPE, COMM, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNTS(*)}, \mbox{SDISPLS(*)}, \mbox{SENDTYPE}, \mbox{RECVCOUNTS(*)}, \mbox{RDISPLS(*)}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLTOALLW}{(\mbox{sendbuf}, \mbox{sendcounts}, \mbox{sdispls}, \mbox{sendtypes}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{rdispls}, \mbox{recvtypes}, \mbox{comm})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcounts}{nonnegative integer array (of length outdegree) specifying the number of elements to send to each neighbor}
\funcarg{\IN}{sdispls}{integer array (of length outdegree). Entry \mpicode{j} specifies the displacement in bytes (relative to \mpiarg{sendbuf}) from which to take the outgoing data destined for neighbor \mpicode{j} (array of integers)}
\funcarg{\IN}{sendtypes}{array of datatypes (of length outdegree). Entry \mpicode{j} specifies the type of data to send to neighbor \mpicode{j} (array of handles)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
\funcarg{\IN}{recvcounts}{nonnegative integer array (of length indegree) specifying the number of elements that are received from each neighbor}
\funcarg{\IN}{rdispls}{integer array (of length indegree). Entry \mpicode{i} specifies the displacement in bytes (relative to \mpiarg{recvbuf}) at which to place the incoming data from neighbor \mpicode{i} (array of integers)}
\funcarg{\IN}{recvtypes}{array of datatypes (of length indegree). Entry \mpicode{i} specifies the type of data received from neighbor \mpicode{i} (array of handles)}
\funcarg{\IN}{comm}{communicator with associated virtual topology (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_alltoallw}{(\gb{}const~void~*sendbuf, const~int~sendcounts[], const~MPI\_Aint~sdispls[], const~MPI\_Datatype~sendtypes[], void~*recvbuf, const~int~recvcounts[], const~MPI\_Aint~rdispls[], const~MPI\_Datatype~recvtypes[], MPI\_Comm~comm)}
\MPIbindindex{MPI\_Neighbor\_alltoallw\_c}
\mpibindmain{MPI\_Neighbor\_alltoallw\_c}{(\gb{}const~void~*sendbuf, const~MPI\_Count~sendcounts[], const~MPI\_Aint~sdispls[], const~MPI\_Datatype~sendtypes[], void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~rdispls[], const~MPI\_Datatype~recvtypes[], MPI\_Comm~comm)}
\mpifnewbindmain{MPI\_Neighbor\_alltoallw}{(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN)\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtypes(*)}, \mbox{recvtypes(*)}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_alltoallw}{(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN)\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN)\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtypes(*)}, \mbox{recvtypes(*)}\fnarg{}TYPE(*), DIMENSION(..)\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLTOALLW}{(SENDBUF, SENDCOUNTS, SDISPLS, SENDTYPES, RECVBUF, RECVCOUNTS, RDISPLS, RECVTYPES, COMM, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnarg{}INTEGER \mbox{SENDCOUNTS(*)}, \mbox{SENDTYPES(*)}, \mbox{RECVCOUNTS(*)}, \mbox{RECVTYPES(*)}, \mbox{COMM}, \mbox{IERROR}\fnfinalarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND) \mbox{SDISPLS(*)}, \mbox{RDISPLS(*)}}
...
\begin{funcdef}{MPI\_INEIGHBOR\_ALLGATHER}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcount}, \mbox{recvtype}, \mbox{comm}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
...
\end{funcdef}
\mpibindmain{MPI\_Ineighbor\_allgather}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, int~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\MPIbindindex{MPI\_Ineighbor\_allgather\_c}
\mpibindmain{MPI\_Ineighbor\_allgather\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, MPI\_Count~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Ineighbor\_allgather}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
...
\mpifbindmain{MPI\_INEIGHBOR\_ALLGATHER}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, COMM, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNT}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{REQUEST}, \mbox{IERROR}}

\mpifuncskip{MPI\_INEIGHBOR\_ALLGATHER} starts a nonblocking variant of
\mpifunc{MPI\_NEIGHBOR\_ALLGATHER}.

\begin{funcdef}{MPI\_INEIGHBOR\_ALLGATHERV}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{displs}, \mbox{recvtype}, \mbox{comm}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
\funcarg{\IN}{recvcounts}{nonnegative integer array (of length indegree) containing the number of elements that are received from each neighbor}
\funcarg{\IN}{displs}{integer array (of length indegree). Entry \mpicode{i} specifies the displacement (relative to \mpiarg{recvbuf}) at rwhich to place the incoming data from neighbor \mpicode{i}}
\funcarg{\IN}{recvtype}{datatype of receive buffer elements (handle)}
\funcarg{\IN}{comm}{communicator with associated virtual topology (handle)}
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Ineighbor\_allgatherv}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, const~int~recvcounts[], const~int~displs[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\MPIbindindex{MPI\_Ineighbor\_allgatherv\_c}
\mpibindmain{MPI\_Ineighbor\_allgatherv\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~displs[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Ineighbor\_allgatherv}{(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}INTEGER, INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{recvcounts(*)}, \mbox{displs(*)}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
...
\mpifbindmain{MPI\_INEIGHBOR\_ALLGATHERV}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNTS, DISPLS, RECVTYPE, COMM, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNTS(*)}, \mbox{DISPLS(*)}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{REQUEST}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_INEIGHBOR\_ALLTOALL}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcount}, \mbox{recvtype}, \mbox{comm}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Ineighbor\_alltoall}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, int~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\MPIbindindex{MPI\_Ineighbor\_alltoall\_c}
\mpibindmain{MPI\_Ineighbor\_alltoall\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, MPI\_Count~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Ineighbor\_alltoall}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Ineighbor\_alltoall}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, request, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_INEIGHBOR\_ALLTOALL}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, COMM, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNT}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{REQUEST}, \mbox{IERROR}}

\mpifuncskip{MPI\_INEIGHBOR\_ALLTOALL} starts a nonblocking variant of
\mpifunc{MPI\_NEIGHBOR\_ALLTOALL}.


\begin{funcdef}{MPI\_INEIGHBOR\_ALLTOALLV}{(\mbox{sendbuf}, \mbox{sendcounts}, \mbox{sdispls}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{rdispls}, \mbox{recvtype}, \mbox{comm}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcounts}{nonnegative integer array (of length outdegree) specifying the number of elements to send to each neighbor}
\funcarg{\IN}{sdispls}{integer array (of length outdegree). Entry \mpicode{j} specifies the displacement (relative to \mpiarg{sendbuf}) from which send the outgoing data to neighbor \mpicode{j}}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
\funcarg{\IN}{recvcounts}{nonnegative integer array (of length indegree) specifying the number of elements that are received from each neighbor}
\funcarg{\IN}{rdispls}{integer array (of length indegree). Entry \mpicode{i} specifies the displacement (relative to \mpiarg{recvbuf}) at which to place the incoming data from neighbor \mpicode{i}}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Ineighbor\_alltoallv}{(\gb{}const~void~*sendbuf, const~int~sendcounts[], const~int~sdispls[], MPI\_Datatype~sendtype, void~*recvbuf, const~int~recvcounts[], const~int~rdispls[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\MPIbindindex{MPI\_Ineighbor\_alltoallv\_c}
\mpibindmain{MPI\_Ineighbor\_alltoallv\_c}{(\gb{}const~void~*sendbuf, const~MPI\_Count~sendcounts[], const~MPI\_Aint~sdispls[], MPI\_Datatype~sendtype, void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~rdispls[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Ineighbor\_alltoallv}{(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype, comm, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{sdispls(*)}, \mbox{recvcounts(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
...
\mpifbindmain{MPI\_INEIGHBOR\_ALLTOALLV}{(SENDBUF, SENDCOUNTS, SDISPLS, SENDTYPE, RECVBUF, RECVCOUNTS, RDISPLS, RECVTYPE, COMM, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNTS(*)}, \mbox{SDISPLS(*)}, \mbox{SENDTYPE}, \mbox{RECVCOUNTS(*)}, \mbox{RDISPLS(*)}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{REQUEST}, \mbox{IERROR}}

\mpifuncskip{MPI\_INEIGHBOR\_ALLTOALLV} starts a nonblocking variant of
\mpifunc{MPI\_NEIGHBOR\_ALLTOALLV}.

\begin{funcdef}{MPI\_INEIGHBOR\_ALLTOALLW}{(\mbox{sendbuf}, \mbox{sendcounts}, \mbox{sdispls}, \mbox{sendtypes}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{rdispls}, \mbox{recvtypes}, \mbox{comm}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcounts}{nonnegative integer array (of length outdegree) specifying the number of elements to send to each neighbor}
\funcarg{\IN}{sdispls}{integer array (of length outdegree). Entry \mpicode{j} specifies the displacement in bytes (relative to \mpiarg{sendbuf}) from which to take the outgoing data destined for neighbor \mpicode{j} (array of integers)}
\funcarg{\IN}{sendtypes}{array of datatypes (of length outdegree). Entry \mpicode{j} specifies the type of data to send to neighbor \mpicode{j} (array of handles)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Ineighbor\_alltoallw}{(\gb{}const~void~*sendbuf, const~int~sendcounts[], const~MPI\_Aint~sdispls[], const~MPI\_Datatype~sendtypes[], void~*recvbuf, const~int~recvcounts[], const~MPI\_Aint~rdispls[], const~MPI\_Datatype~recvtypes[], MPI\_Comm~comm, MPI\_Request~*request)}
\MPIbindindex{MPI\_Ineighbor\_alltoallw\_c}
\mpibindmain{MPI\_Ineighbor\_alltoallw\_c}{(\gb{}const~void~*sendbuf, const~MPI\_Count~sendcounts[], const~MPI\_Aint~sdispls[], const~MPI\_Datatype~sendtypes[], void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~rdispls[], const~MPI\_Datatype~recvtypes[], MPI\_Comm~comm, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Ineighbor\_alltoallw}{(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendtypes(*)}, \mbox{recvtypes(*)}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Ineighbor\_alltoallw}{(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, request, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendtypes(*)}, \mbox{recvtypes(*)}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_INEIGHBOR\_ALLTOALLW}{(SENDBUF, SENDCOUNTS, SDISPLS, SENDTYPES, RECVBUF, RECVCOUNTS, RDISPLS, RECVTYPES, COMM, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnarg{}INTEGER \mbox{SENDCOUNTS(*)}, \mbox{SENDTYPES(*)}, \mbox{RECVCOUNTS(*)}, \mbox{RECVTYPES(*)}, \mbox{COMM}, \mbox{REQUEST}, \mbox{IERROR}\fnfinalarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND) \mbox{SDISPLS(*)}, \mbox{RDISPLS(*)}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLGATHER\_INIT}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcount}, \mbox{recvtype}, \mbox{comm}, \mbox{info}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_allgather\_init}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, int~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\MPIbindindex{MPI\_Neighbor\_allgather\_init\_c}
\mpibindmain{MPI\_Neighbor\_allgather\_init\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, MPI\_Count~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Neighbor\_allgather\_init}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, info, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
...
\mpifbindmain{MPI\_NEIGHBOR\_ALLGATHER\_INIT}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, COMM, INFO, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNT}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{INFO}, \mbox{REQUEST}, \mbox{IERROR}}

Creates a persistent collective communication request for the neighborhood allgather operation. 

\begin{funcdef}{MPI\_NEIGHBOR\_ALLGATHERV\_INIT}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{displs}, \mbox{recvtype}, \mbox{comm}, \mbox{info}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_allgatherv\_init}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, const~int~recvcounts[], const~int~displs[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\MPIbindindex{MPI\_Neighbor\_allgatherv\_init\_c}
\mpibindmain{MPI\_Neighbor\_allgatherv\_init\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~displs[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Neighbor\_allgatherv\_init}{(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm, info, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{displs(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}INTEGER, INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{recvcounts(*)}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_allgatherv\_init}{(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm, info, request, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN)\ ::\ \mbox{displs(*)}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLGATHERV\_INIT}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNTS, DISPLS, RECVTYPE, COMM, INFO, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNTS(*)}, \mbox{DISPLS(*)}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{INFO}, \mbox{REQUEST}, \mbox{IERROR}}
...
\begin{funcdef}{MPI\_NEIGHBOR\_ALLTOALL\_INIT}{(\mbox{sendbuf}, \mbox{sendcount}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcount}, \mbox{recvtype}, \mbox{comm}, \mbox{info}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcount}{number of elements sent to each neighbor (nonnegative integer)}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
\funcarg{\OUT}{recvbuf}{starting address of receive buffer (choice)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_alltoall\_init}{(\gb{}const~void~*sendbuf, int~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, int~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\MPIbindindex{MPI\_Neighbor\_alltoall\_init\_c}
\mpibindmain{MPI\_Neighbor\_alltoall\_init\_c}{(\gb{}const~void~*sendbuf, MPI\_Count~sendcount, MPI\_Datatype~sendtype, void~*recvbuf, MPI\_Count~recvcount, MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Neighbor\_alltoall\_init}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, info, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_alltoall\_init}{(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, info, request, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN)\ ::\ \mbox{sendcount}, \mbox{recvcount}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLTOALL\_INIT}{(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, COMM, INFO, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNT}, \mbox{SENDTYPE}, \mbox{RECVCOUNT}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{INFO}, \mbox{REQUEST}, \mbox{IERROR}}

Creates a persistent collective communication request for the neighborhood alltoall operation. 

\begin{funcdef}{MPI\_NEIGHBOR\_ALLTOALLV\_INIT}{(\mbox{sendbuf}, \mbox{sendcounts}, \mbox{sdispls}, \mbox{sendtype}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{rdispls}, \mbox{recvtype}, \mbox{comm}, \mbox{info}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcounts}{nonnegative integer array (of length outdegree) specifying the number of elements to send to each neighbor}
\funcarg{\IN}{sdispls}{integer array (of length outdegree). Entry \mpicode{j} specifies the displacement (relative to \mpiarg{sendbuf}) from which send the outgoing data to neighbor \mpicode{j}}
\funcarg{\IN}{sendtype}{datatype of send buffer elements (handle)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_alltoallv\_init}{(\gb{}const~void~*sendbuf, const~int~sendcounts[], const~int~sdispls[], MPI\_Datatype~sendtype, void~*recvbuf, const~int~recvcounts[], const~int~rdispls[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\MPIbindindex{MPI\_Neighbor\_alltoallv\_init\_c}
\mpibindmain{MPI\_Neighbor\_alltoallv\_init\_c}{(\gb{}const~void~*sendbuf, const~MPI\_Count~sendcounts[], const~MPI\_Aint~sdispls[], MPI\_Datatype~sendtype, void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~rdispls[], MPI\_Datatype~recvtype, MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Neighbor\_alltoallv\_init}{(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype, comm, info, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{sdispls(*)}, \mbox{recvcounts(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_alltoallv\_init}{(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype, comm, info, request, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN)\ ::\ \mbox{sendtype}, \mbox{recvtype}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLTOALLV\_INIT}{(SENDBUF, SENDCOUNTS, SDISPLS, SENDTYPE, RECVBUF, RECVCOUNTS, RDISPLS, RECVTYPE, COMM, INFO, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnfinalarg{}INTEGER \mbox{SENDCOUNTS(*)}, \mbox{SDISPLS(*)}, \mbox{SENDTYPE}, \mbox{RECVCOUNTS(*)}, \mbox{RDISPLS(*)}, \mbox{RECVTYPE}, \mbox{COMM}, \mbox{INFO}, \mbox{REQUEST}, \mbox{IERROR}}

Creates a persistent collective communication request for the neighborhood alltoallv operation. 


\begin{funcdef}{MPI\_NEIGHBOR\_ALLTOALLW\_INIT}{(\mbox{sendbuf}, \mbox{sendcounts}, \mbox{sdispls}, \mbox{sendtypes}, \mbox{recvbuf}, \mbox{recvcounts}, \mbox{rdispls}, \mbox{recvtypes}, \mbox{comm}, \mbox{info}, \mbox{request})}
\funcarg{\IN}{sendbuf}{starting address of send buffer (choice)}
\funcarg{\IN}{sendcounts}{nonnegative integer array (of length outdegree) specifying the number of elements to send to each neighbor}
\funcarg{\IN}{sdispls}{integer array (of length outdegree). Entry \mpicode{j} specifies the displacement in bytes (relative to \mpiarg{sendbuf}) from which to take the outgoing data destined for neighbor \mpicode{j} (array of integers)}
\funcarg{\IN}{sendtypes}{array of datatypes (of length outdegree). Entry \mpicode{j} specifies the type of data to send to neighbor \mpicode{j} (array of handles)}
...
\funcarg{\OUT}{request}{communication request (handle)}
\end{funcdef}
\mpibindmain{MPI\_Neighbor\_alltoallw\_init}{(\gb{}const~void~*sendbuf, const~int~sendcounts[], const~MPI\_Aint~sdispls[], const~MPI\_Datatype~sendtypes[], void~*recvbuf, const~int~recvcounts[], const~MPI\_Aint~rdispls[], const~MPI\_Datatype~recvtypes[], MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\MPIbindindex{MPI\_Neighbor\_alltoallw\_init\_c}
\mpibindmain{MPI\_Neighbor\_alltoallw\_init\_c}{(\gb{}const~void~*sendbuf, const~MPI\_Count~sendcounts[], const~MPI\_Aint~sdispls[], const~MPI\_Datatype~sendtypes[], void~*recvbuf, const~MPI\_Count~recvcounts[], const~MPI\_Aint~rdispls[], const~MPI\_Datatype~recvtypes[], MPI\_Comm~comm, MPI\_Info~info, MPI\_Request~*request)}
\mpifnewbindmain{MPI\_Neighbor\_alltoallw\_init}{(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, info, request, ierror) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER, INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendtypes(*)}, \mbox{recvtypes(*)}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifnewbindmain{MPI\_Neighbor\_alltoallw\_init}{(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, info, request, ierror) !(\_c) \fargs TYPE(*), DIMENSION(..), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendbuf}\fnarg{}INTEGER(KIND=MPI\_COUNT\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendcounts(*)}, \mbox{recvcounts(*)}\fnarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sdispls(*)}, \mbox{rdispls(*)}\fnarg{}TYPE(MPI\_Datatype), INTENT(IN), ASYNCHRONOUS\ ::\ \mbox{sendtypes(*)}, \mbox{recvtypes(*)}\fnarg{}TYPE(*), DIMENSION(..), ASYNCHRONOUS\ ::\ \mbox{recvbuf}\fnarg{}TYPE(MPI\_Comm), INTENT(IN)\ ::\ \mbox{comm}\fnarg{}TYPE(MPI\_Info), INTENT(IN)\ ::\ \mbox{info}\fnarg{}TYPE(MPI\_Request), INTENT(OUT)\ ::\ \mbox{request}\fnfinalarg{}INTEGER, OPTIONAL, INTENT(OUT)\ ::\ \mbox{ierror}}
\mpifbindmain{MPI\_NEIGHBOR\_ALLTOALLW\_INIT}{(SENDBUF, SENDCOUNTS, SDISPLS, SENDTYPES, RECVBUF, RECVCOUNTS, RDISPLS, RECVTYPES, COMM, INFO, REQUEST, IERROR) \fargs <type> \mbox{SENDBUF(*)}, \mbox{RECVBUF(*)}\fnarg{}INTEGER \mbox{SENDCOUNTS(*)}, \mbox{SENDTYPES(*)}, \mbox{RECVCOUNTS(*)}, \mbox{RECVTYPES(*)}, \mbox{COMM}, \mbox{INFO}, \mbox{REQUEST}, \mbox{IERROR}\fnfinalarg{}INTEGER(KIND=MPI\_ADDRESS\_KIND) \mbox{SDISPLS(*)}, \mbox{RDISPLS(*)}}
