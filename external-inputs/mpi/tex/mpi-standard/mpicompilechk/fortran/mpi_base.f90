...
! Here begins routines with choice arguments.  A Fortran 2008 compiler
! is needed to compile these
! Also included are routines missing in the above

        subroutine mpi_get_address(x,a,ierr)
          use MPI_CONSTANTS, ONLY: MPI_ADDRESS_KIND
          type (*), dimension(..) :: x
          integer(kind=MPI_ADDRESS_KIND) a
          integer ierr
        end subroutine mpi_get_address

        subroutine mpi_send(buf, cnt, dtype, dest, tag, comm, ierr)
          type (*), dimension(..) :: buf
          integer cnt, dtype, dest, tag, comm, ierr
        end subroutine mpi_send

        subroutine mpi_ssend(buf, cnt, dtype, dest, tag, comm, ierr)
          type (*), dimension(..) :: buf
          integer cnt, dtype, dest, tag, comm, ierr
        end subroutine mpi_ssend

        subroutine mpi_bsend(buf, cnt, dtype, dest, tag, comm, ierr)
          type (*), dimension(..) :: buf
          integer cnt, dtype, dest, tag, comm, ierr
        end subroutine mpi_bsend

        subroutine mpi_recv(buf, cnt, dtype, dest, tag, comm, status, ierr)
          use MPI_CONSTANTS, ONLY: MPI_STATUS_SIZE
          type (*), dimension(..) :: buf
          integer cnt, dtype, dest, tag, comm, ierr
          integer status(MPI_STATUS_SIZE)
        ...
        subroutine mpi_isend(buf, cnt, dtype, dest, tag, comm, req, ierr)
          type (*), dimension(..) :: buf
          integer cnt, dtype, dest, tag, comm, ierr, req
        end subroutine mpi_isend

        subroutine mpi_irecv(buf, cnt, dtype, dest, tag, comm, req, ierr)
          type (*), dimension(..) :: buf
          integer cnt, dtype, dest, tag, comm, ierr
          integer req
...
! Attributes
        ...
        subroutine mpi_attr_get(comm, comm_keyval, attribute_val, flag, &
             ierror)
          integer comm, comm_keyval, ierror
        ...
        subroutine mpi_attr_put(comm, comm_keyval, attribute_val, ierror)
          integer comm, comm_keyval, ierror
          integer attribute_val
        end subroutine mpi_attr_put

! RMA routines
        subroutine mpi_win_create(base, size, disp_unit, info, comm, win, &
             ierror)
          use MPI_CONSTANTS, ONLY: MPI_ADDRESS_KIND
          type (*), dimension(..) :: base
          integer(kind=MPI_ADDRESS_KIND) size
...
! Collectives
        ...
        subroutine mpi_neighbor_alltoallw(sendbuf, sendcounts, sdispls, &
             sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm, ierror)
          use MPI_CONSTANTS, ONLY: MPI_ADDRESS_KIND
          type (*), dimension(..) ::SENDBUF, RECVBUF
        ...
        subroutine mpi_neighbor_alltoallw_init(sendbuf, sendcounts, sdispls, &
