...
\cdeclmainindex{MPI\_Comm}
\begin{mpi-binding}
    function_name("MPI_Comm_is_revoked")
    parameter("comm", "COMMUNICATOR")
    parameter("flag", "LOGICAL", desc=r"\mpicode{true} if the communicator is revoked", direction="out")
\end{mpi-binding}
\mpitermindex{error handling!fault tolerance!revoked}
...
\begin{mpi-binding}
    function_name("MPI_Comm_get_failed")
    parameter("comm", "COMMUNICATOR")
    parameter("failedgrp", "GROUP", direction="out", desc=r"group of failed processes")
\end{mpi-binding}
...
\cdeclmainindex{MPI\_Comm}
\begin{mpi-binding}
    function_name("MPI_Comm_ack_failed")
    parameter("comm", "COMMUNICATOR")
    parameter("num_to_ack", "COMM_SIZE", desc=r"Maximum number of process failures to acknowledge")
    parameter("num_acked", "COMM_SIZE", direction="out", desc=r"Number of process failures acknowledged")
\end{mpi-binding}
